...
最后基于当前代码检查日期：2026-05-18

## 1. 整体进度

NBC 钱包功能目前以 EverShop 扩展的方式接入，扩展目录为 `extensions/nbc-wallet`。

当前状态：后端主链路已经基本完成。扩展可以注册 NBC 钱包支付方式，通过钱包签名创建或登录绑定钱包的客户账号，维护 NBC 余额与账本，完成订单钱包扣款、订单退款、后台人工调账，并在链上配置启用后处理链上充值事件。

前端状态：目前是部分接入。已有 checkout 支付组件，可以在订单创建后触发 NBC 钱包扣款；但完整的钱包连接/登录入口、钱包首页、余额页、交易流水页等 UI 还没有在当前代码中看到。
