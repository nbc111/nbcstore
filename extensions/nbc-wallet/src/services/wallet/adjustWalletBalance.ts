...
function buildWalletLookup(input: AdjustWalletBalanceInput) {
  if (input.walletId) {
    return {
      sql: 'wallet_id = $1',
      params: [input.walletId] as Array<string | number>
    };
  }
  if (input.customerId) {
    return {
      sql: 'customer_id = $1',
      params: [input.customerId] as Array<string | number>
    };
  }
  if (input.walletAddress) {
    return {
      sql: 'wallet_address = $1',
      params: [normalizeWalletAddress(input.walletAddress)] as Array<string | number>
    };
  }
  throw new Error('walletId, customerId or walletAddress is required');
}

export async function adjustWalletBalance(input: AdjustWalletBalanceInput) {
  ...
  try {
    ...
    const walletResult = await connection.query(
      `SELECT * FROM nbc_wallet WHERE ${lookup.sql} FOR UPDATE`,
      lookup.params
    );
