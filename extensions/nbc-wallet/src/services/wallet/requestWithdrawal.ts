...
export async function requestWithdrawal(input: RequestWithdrawalInput) {
  ...
  if (amount < limits.minAmount) {
    throw new Error(
      `Withdrawal amount must be at least ${limits.minAmount} NBC`
    );
  }

  if (limits.maxAmount > 0 && amount > limits.maxAmount) {
    throw new Error(
      `Withdrawal amount must not exceed ${limits.maxAmount} NBC per transaction`
    );
  }
  ...
  try {
    ...
    if (limits.dailyLimit > 0) {
      ...
      if (dailyTotal + amount > limits.dailyLimit) {
        ...
        throw new Error(
          `Daily withdrawal limit of ${limits.dailyLimit} NBC exceeded. ` +
