...
function getWithdrawalLimits(assetSymbol = 'NBC') {
  const prefix =
    assetSymbol === 'USDT'
      ? 'nbcWallet.assets.USDT.withdrawal'
      : 'nbcWallet.withdrawal';
  return {
    ...
    frequencyWindowSeconds: Number(
      getConfig(`${prefix}.frequencyWindowSeconds`, 3600)
    ),
    maxRequestsPerWindow: Number(getConfig(`${prefix}.maxRequestsPerWindow`, 3))
  };
}

function getConfiguredList(path: string): string[] {
...
export async function requestWithdrawal(input: RequestWithdrawalInput) {
  ...
  if (amount < limits.minAmount) {
    throw new Error(
      `Withdrawal amount must be at least ${limits.minAmount} ${asset.symbol}`
    );
  }

  if (limits.maxAmount > 0 && amount > limits.maxAmount) {
    throw new Error(
      `Withdrawal amount must not exceed ${limits.maxAmount} ${asset.symbol} per transaction`
    );
  }
  ...
  try {
    ...
    const normalizedWithdrawalAddress = String(
      lockedWallet.wallet_address || ''
    ).toLowerCase();
    const blacklist = getConfiguredList(
      `nbcWallet.withdrawal.blacklistedAddresses`
    ).map((item) => item.toLowerCase());
    const assetBlacklist = getConfiguredList(
      `nbcWallet.assets.${asset.symbol}.withdrawal.blacklistedAddresses`
    ).map((item) => item.toLowerCase());

    if (
      blacklist.includes(normalizedWithdrawalAddress) ||
      assetBlacklist.includes(normalizedWithdrawalAddress)
    ) {
    ...
    if (limits.dailyLimit > 0) {
      ...
      if (dailyTotal + amount > limits.dailyLimit) {
        ...
        throw new Error(
          `Daily withdrawal limit of ${limits.dailyLimit} ${asset.symbol} exceeded. ` +
            `Remaining today: ${remaining} ${asset.symbol}`
        );
      }
    }

    if (limits.maxRequestsPerWindow > 0 && limits.frequencyWindowSeconds > 0) {
      const frequencyResult = await connection.query(
      ...
      const requestCount = Number(
        frequencyResult.rows[0]?.request_count || 0
      );
    ...
    await connection.query(
      ...
      [nextFrozenBalance, assetBalance.wallet_asset_id]
    );
