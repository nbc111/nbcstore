 ...
 */
async function reconcileDeposits(limit: number) {
      ...
      await pool.query(
        `UPDATE nbc_onchain_deposit
            SET status = 'failed',
                ...
                updated_at = NOW()
          WHERE deposit_id = $2`,
        [error instanceof Error ? error.message : String(error), row.deposit_id]
      );
    }
  }
...
async function reconcileProcessingWithdrawals(staleAfterMinutes = 30) {
  ...
  const result = await pool.query(
    `SELECT w.*, nw.balance, nw.frozen_balance
  ...
  for (const row of result.rows) {
        ...
        if (receipt && receipt.status === 1) {
          ...
          try {
            ...
            await conn.query(
              ...
              [walletTx.insertId || walletTx.wallet_tx_id, row.withdrawal_id]
            );
    ...
    try {
      ...
      await conn2.query(
        `UPDATE nbc_wallet SET balance = $1, updated_at = NOW() WHERE wallet_id = $2`,
        [restoredBalance, row.wallet_id]
      );
