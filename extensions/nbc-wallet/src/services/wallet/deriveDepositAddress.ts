...
export function deriveDepositAddress(index: number) {
  ...
  if (config.depositXpub) {
    return normalizeWalletAddress(
      HDNodeWallet.fromExtendedKey(config.depositXpub).deriveChild(index).address
    );
  }

  if (config.depositMnemonic) {
    return normalizeWalletAddress(
      HDNodeWallet.fromPhrase(config.depositMnemonic, undefined, childPath).address
    );
  }
