...
export function getOnchainConfig(assetSymbol = 'NBC') {
    const chain = getChainRpcConfig();
    const asset = getWalletAssetConfig(assetSymbol);
    const treasuryAddress = String(process.env.NBC_WALLET_TREASURY_ADDRESS || getConfig('nbcWallet.onchain.treasuryAddress', ''));
    const depositMode = String(process.env.NBC_WALLET_DEPOSIT_MODE || getConfig('nbcWallet.onchain.deposit.mode', getConfig('nbcWallet.onchain.depositMode', 'treasury'))).toLowerCase();
    return {
        enabled: Number(process.env.NBC_WALLET_ONCHAIN_ENABLED ?? getConfig('nbcWallet.onchain.enabled', 1)) === 1,
        rpcUrl: chain.rpcUrl,
        ...
        hdPathPrefix: String(process.env.NBC_WALLET_HD_PATH_PREFIX || getConfig('nbcWallet.onchain.hdPathPrefix', getConfig('nbcWallet.onchain.deposit.derivationPath', "m/44'/60'/0'/0"))).replace(/\/+$/, ''),
        depositXpub: String(process.env.NBC_WALLET_DEPOSIT_XPUB || getConfig('nbcWallet.onchain.deposit.xpub', '')).trim(),
        depositMnemonic: String(process.env.NBC_WALLET_DEPOSIT_MNEMONIC || getConfig('nbcWallet.onchain.deposit.mnemonic', '')).trim(),
        depositDerivationPath: String(process.env.NBC_WALLET_DEPOSIT_DERIVATION_PATH || getConfig('nbcWallet.onchain.deposit.derivationPath', "m/44'/60'/0'/0")).replace(/\/+$/, ''),
...
export function getOnchainConfigs() {
...
export function assertOnchainConfig(config = getOnchainConfig()) {
    ...
    if (config.confirmations <= 0 && ![
        'development',
        'test'
    ].includes(String(process.env.NODE_ENV || ''))) {
        throw new Error('nbcWallet.onchain.confirmations must be greater than 0');
    }
    if (config.depositMode === 'hd' && !config.depositXpub && !config.depositMnemonic) {
