...
async function reconcileDeposits(limit) {
    const result = await pool.query(`SELECT deposit_id
      ...
      LIMIT $1`, [Math.max(Number(limit) || 100, 1)]);
    let completed = 0;
    let unmatched = 0;
    let failed = 0;
    for (const row of result.rows) {
        try {
            ...
            SET status = 'failed',
                error_message = $1,
                updated_at = NOW()
          WHERE deposit_id = $2`, [error instanceof Error ? error.message : String(error), row.deposit_id]);
        }
    }
    return { scanned: result.rows.length, completed, unmatched, failed };
}
/**
...
async function reconcileProcessingWithdrawals(staleAfterMinutes = 30) {
    ...
    for (const row of result.rows) {
            ...
            try {
                ...
                if (receipt && receipt.status === 1) {
                  ...
                  SET status = 'completed', processed_at = NOW(), updated_at = NOW(),
                      wallet_tx_id = $1
                WHERE withdrawal_id = $2`, [walletTx.insertId || walletTx.wallet_tx_id, row.withdrawal_id]);
                        await commit(conn);
                        finalized += 1;
                        await Promise.all([
                ...
                // Cannot reach chain — skip, will retry next cycle
                continue;
            }
        }
        ...
        try {
            ...
            await conn2.query(`UPDATE nbc_wallet SET balance = $1, updated_at = NOW() WHERE wallet_id = $2`, [restoredBalance, row.wallet_id]);
            const failReason = row.tx_hash
                ? `Reconcile: tx ${row.tx_hash} not confirmed after ${staleAfterMinutes}m`
                : `Reconcile: stuck in processing without tx_hash after ${staleAfterMinutes}m`;
            await conn2.query(`UPDATE nbc_withdrawal
            SET status = 'failed', failed_at = NOW(), error_message = $1, updated_at = NOW()
          WHERE withdrawal_id = $2`, [failReason, row.withdrawal_id]);
            await commit(conn2);
            restored += 1;
            await Promise.all([
                ...
                writeAuditLog({
                    ...
                    metadata: { tx_hash: row.tx_hash, reason: failReason }
                })
            ]);
        }
        catch (_err) {
    ...
    return { scanned: result.rows.length, finalized, restored };
}
