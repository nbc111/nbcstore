...
function getTokenAddressKey(config) {
    return config.assetType === 'native' ? 'native:NBC' : normalizeWalletAddress(config.tokenAddress);
}
async function settleRecordedDeposit(record) {
    let settled = 0;
    if (record.depositId) {
...
async function processErc20ToTreasury(provider, config, fromBlock, toBlock) {
...
async function processErc20ToHdAddresses(provider, config, fromBlock, toBlock) {
    ...
    for (const binding of bindings){
        const logs = await provider.getLogs({
            ...
            topics: [
                TRANSFER_TOPIC,
                null,
                topicAddress(binding.depositAddress)
            ]
        });
        for (const log of logs){
            const parsed = transferInterface.parseLog({
...
async function processNativeTransfers(provider, config, fromBlock, toBlock) {
    const tokenAddressKey = getTokenAddressKey(config);
    ...
    if (config.depositMode === 'hd') {
        ...
        monitored.set(config.treasuryAddress, {
            mode: 'treasury'
        });
    }
    if (monitored.size === 0) {
    ...
    for(let blockNumber = fromBlock; blockNumber <= toBlock; blockNumber += 1){
        ...
        for (const tx of transactions){
            ...
            const walletAddress = binding.mode === 'hd' ? binding.walletAddress : tx.from ? normalizeWalletAddress(String(tx.from)) : '';
            if (!walletAddress) {
                continue;
            }
            const record = await recordOnchainDeposit({
