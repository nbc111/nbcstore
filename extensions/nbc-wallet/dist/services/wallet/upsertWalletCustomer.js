...
export async function upsertWalletCustomer(walletAddress) {
    ...
    try {
        ...
        const existingWallet = await select().from('nbc_wallet').where('wallet_address', '=', normalizedAddress).load(connection);
        if (existingWallet) {
            const existingCustomer = await select().from('customer').where('customer_id', '=', existingWallet.customer_id).load(connection);
            if (!existingCustomer) {
        ...
        const existingCustomerByEmail = await select().from('customer').where('email', '=', customerEmail).load(connection);
        if (existingCustomerByEmail) {
            throw new Error('Shadow customer email conflict');
        }
        const customer = await insert('customer').given({
            email: customerEmail,
        ...
        const createdCustomer = await select().from('customer').where('customer_id', '=', customer.insertId).load(connection);
        await commit(connection);
