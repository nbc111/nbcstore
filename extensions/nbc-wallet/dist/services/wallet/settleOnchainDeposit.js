...
import { getAssetSymbolByTokenAddress, getWalletAssetConfig } from './assets.js';
...
export async function settleOnchainDeposit(depositId) {
    ...
    try {
        ...
        const assetSymbol = deposit.asset_symbol || getAssetSymbolByTokenAddress(deposit.token_address);
        ...
        await connection.query(`UPDATE nbc_wallet_asset_balance
          SET balance = $1,
              updated_at = NOW()
        WHERE wallet_asset_id = $2`, [balanceAfter.toString(), assetBalance.wallet_asset_id]);
        if (asset.symbol === 'NBC') {
        ...
        await connection.query(`UPDATE nbc_onchain_deposit
          SET wallet_id = $1,
              ...
              processed_at = NOW(),
              updated_at = NOW()
        WHERE deposit_id = $5`, [wallet.wallet_id, walletTxId, asset.symbol, tokenDecimals, depositId]);
        await commit(connection);
        const result = {
            depositId,
