...
export const DefaultMiniCartIcon = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`web3-icon-btn mini-cart-icon relative ${
    ...
    >
      ...
      {showItemCount && totalQty > 0 && !syncStatus.syncing && (
        <span
          className={`absolute -top-1.5 -right-1.5 bg-primary text-primary-foreground text-[10px] rounded-full min-w-4 h-4 px-1 flex items-center justify-center font-semibold ${
            badgeAnimating ? 'web3-cart-badge' : ''
          }`}
        >
