...
class JobManager {
   ...
   */ _ensureMutable() {
        if (this._isFrozen) {
            throw new Error('Job manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a job from the bootstrap file.');
        }
    }
    /**
   ...
   */ registerJob(job) {
        ...
        if (!job || typeof job.name !== 'string' || job.name.trim() === '') {
            throw new Error('Cannot register job. Job object must have a valid "name" property.');
        }
        const jobName = job.name;
        if (this.jobs.has(jobName)) {
            warning(`Job with name "${jobName}" is already registered. Skipping registration.`);
            return false;
        }
        if (!isValidJsFilePath(job.resolve)) {
            throw new Error(`Cannot register job "${jobName}". Invalid or unresolvable path: "${job.resolve}". Please ensure it's a valid path to an existing JS file.`);
        }
        if (!cron.validate(job.schedule)) {
            throw new Error(`Cannot register job "${jobName}". Invalid cron schedule: "${job.schedule}". Please ensure it's a valid cron expression.`);
        }
   ...
   */ updateJobSchedule(jobName, newSchedule) {
        ...
        if (!cron.validate(newSchedule)) {
            throw new Error(`Cannot update job "${jobName}". Invalid cron schedule: "${newSchedule}". Please ensure it's a valid cron expression.`);
        }
        if (!job) {
   ...
   */ getAllJobs() {
        ...
        return Array.from(this.jobs.values()).map((job)=>Object.freeze({
                ...job
            }));
    }
    /**
 ...
 */ export function updateJobSchedule(jobName, newSchedule) {
 ...
 */ export function removeJob(jobName) {
 ...
 */ export function hasJob(jobName) {
