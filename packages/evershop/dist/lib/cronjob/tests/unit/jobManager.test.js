...
describe('Job Manager Module', ()=>{
    ...
    describe('Mutation after getAllJobs()', ()=>{
        it('should throw an error if resolve is not a string', async ()=>{
            ...
            expect(()=>jobModule.registerJob(invalidJob)).toThrow('Invalid or unresolvable');
        });
        ...
        it('should throw an error if registerJob is called with an invalid job schedule', async ()=>{
            ...
            expect(()=>jobModule.registerJob(invalidJob)).toThrow('Invalid cron schedule');
        });
        ...
        it('should throw an error if updateJob is called after getAllJobs', async ()=>{
            ...
            jobModule.getAllJobs(); // This freezes the manager
            expect(()=>jobModule.updateJobSchedule('ExistingJob', '0 0 * * *')).toThrow('Job manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a job from the bootstrap file.');
        });
        it('should throw an error if removeJob is called after getAllJobs', async ()=>{
            const jobModule = await import('../../jobManager.js');
            jobModule.registerJob(getValidJob('JobToRemove'));
            jobModule.getAllJobs(); // This freezes the manager
            expect(()=>jobModule.removeJob('JobToRemove')).toThrow('Job manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a job from the bootstrap file.');
        });
        ...
        it('should throw error if trying to updateJobSchedule with invalid schedule', async ()=>{
            ...
            expect(()=>jobModule.updateJobSchedule('JobWithInvalidSchedule', 'invalid_schedule')).toThrow('Invalid cron schedule');
        });
        it('should thrown an error if trying to update job schedule after calling getAllJobs', async ()=>{
            ...
            jobModule.getAllJobs(); // This freezes the manager
            expect(()=>jobModule.updateJobSchedule('JobToUpdate', '0 0 * * *')).toThrow('Job manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a job from the bootstrap file.');
        });
