...
function scanForComponents(path) {
    return readdirSync(resolve(path), {
        withFileTypes: true
    }).filter((dirent)=>dirent.isFile() && /.js$/.test(dirent.name) && /^[A-Z]/.test(dirent.name[0])).map((dirent)=>resolve(path, dirent.name));
}
function scanRouteComponents(route, modules, themePath = null) {
    ...
    modules.forEach((module)=>{
        ...
        const pages = existsSync(rootPath) ? readdirSync(rootPath, {
            withFileTypes: true
    ...
    if (!route.isAdmin && themePath) {
        const themePages = existsSync(resolve(themePath, 'pages')) ? readdirSync(resolve(themePath, 'pages'), {
            withFileTypes: true
        }).filter((dirent)=>dirent.isDirectory()).map((dirent)=>dirent.name) : [];
