...
export function getComponentsByRoute(route) {
    ...
    let components;
    if (theme) {
        components = Object.values(scanRouteComponents(route, modules, resolve(theme.path, 'dist')));
    } else {
    ...
    if (!route.isAdmin) {
        ...
        // Add widgets to components
        return components.concat((widgets || []).map((widget)=>widget.settingComponent));
    }
}
/**
 * Scan components for all routes
 * @returns A map of route IDs to their components
 */ export function getAllRouteComponents(isAdmin = false) {
    const allComponents = {};
