...
class Registry {
    ...
    async get(name, initValue, context, validator) {
        if (this.values[name]) {
            ...
            if (isEqual(initValue, this.values[name].initValue) && isEqual(this.values[name].context, context) && Object.prototype.hasOwnProperty.call(this.values[name], 'value')) {
        ...
        for(let i = 0; i < processors.length; i += 1){
            ...
            if (value === undefined) {
                ...
                console.log(`\x1b[33m⚠️ The processor for the value '${name}' is not returning anything. This may cause unexpected behavior.\x1b[0m`);
            }
    ...
    getSync(name, initValue, context, validator) {
        const validateFunc = (value)=>{
            ...
            if (value !== null && typeof value === 'object' && typeof value.then === 'function') {
                throw new Error(`The 'getSync' function does not support async processor. Please use 'get' function instead`);
    ...
    addProcessor(name, callback, priority) {
        if (locked) {
            throw new Error('Registry is locked. Most likely you are trying to add a processor from a middleware. Consider using a bootstrap file to add processors');
        }
    ...
    addFinalProcessor(name, callback) {
        ...
        if (processors.find((p)=>p.priority === 1000)) {
            throw new Error(`There is already a final processor for the value ${name}`);
        }
 ...
 */ export async function getValue(name, initialization, context, validator) {
    let initValue;
    const value = registry.values[name] || {};
