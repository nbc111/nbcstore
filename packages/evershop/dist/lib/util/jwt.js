 ...
 */ export function generateToken(payload, tokenType = TOKEN_TYPES.CUSTOMER, expiresIn) {
    ...
    const defaultExpiry = tokenType === TOKEN_TYPES.ADMIN ? jwtConfig.admin.expiry : jwtConfig.customer.expiry;
    return jwt.sign({
        ...payload,
        tokenType,
 ...
 */ export function generateRefreshToken(payload, tokenType = TOKEN_TYPES.CUSTOMER) {
    ...
    const refreshExpiry = tokenType === TOKEN_TYPES.ADMIN ? jwtConfig.admin.refreshExpiry : jwtConfig.customer.refreshExpiry;
    return jwt.sign({
        ...payload,
        tokenType,
 ...
 */ export function verifyRefreshToken(token, tokenType) {
    // Use separate secret for refresh tokens
    const jwtConfig = getJwtConfig();
    const secret = jwtConfig[tokenType].refreshSecret;
    if (!secret) {
