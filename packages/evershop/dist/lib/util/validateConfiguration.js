...
export function validateConfiguration(config) {
    const ajv = getAjv();
    const configSchema = getValueSync('configurationSchema', {
        type: 'object'
    }, null, (schema)=>{
        ajv.validateSchema(schema);
        if (ajv.errors) {
            throw new Error(`Your configuration schema is not valid: ${ajv.errors[0].instancePath}`);
        } else {
            return true;
        }
    });
...
function errorFormatter(errors) {
    ...
    errors.forEach((error)=>{
        if (error.keyword === 'errorMessage') {
            ...
            messages.push(`${error.instancePath}/${error.params.additionalProperty} is not allowed.`);
        } else if (error.keyword === 'enum') {
            messages.push(`${error.instancePath} must be one of the following values: ${error.params.allowedValues.join(', ')}`);
        } else {
