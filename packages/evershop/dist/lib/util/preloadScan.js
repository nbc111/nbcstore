...
function generatePreloadLinks(images) {
...
export function injectPreloadLinks(html) {
...
export function injectPreloadLinksAfterCharset(html) {
    const preloadImages = extractPreloadImages(html);
    if (preloadImages.length === 0) {
        return html;
    }
...
export function cleanupPreloadAttributes(html) {
    return html.replace(/\s*itemProp=["']preload["']/gi, '');
}
export function processPreloadImages(html) {
