...
export const parseImageSizes = (sizes)=>{
    // Validate input
    ...
    if (trimmedSizes.endsWith('px') && !trimmedSizes.includes(',') && !trimmedSizes.includes('(')) {
        ...
        if (!isNaN(pixelValue) && pixelValue > 0) {
            ...
            return deviceSizes.filter((size)=>size >= pixelValue * 0.5) // Include smaller sizes for efficiency
    ...
    const conditions = trimmedSizes.split(',').map((s)=>s.trim()).filter(Boolean);
...
export const evaluateCondition = (condition, deviceSize)=>{
...
export const evaluateMediaQuery = (mediaQuery, deviceSize)=>{
    // Handle different media query types with improved regex patterns
    if (mediaQuery.includes('max-width')) {
        const maxWidth = parseFloat(mediaQuery.match(/max-width\s*:\s*(\d+(?:\.\d+)?)px/)?.[1] || '0');
        return deviceSize <= maxWidth;
    }
    if (mediaQuery.includes('min-width')) {
        const minWidth = parseFloat(mediaQuery.match(/min-width\s*:\s*(\d+(?:\.\d+)?)px/)?.[1] || '0');
        return deviceSize >= minWidth;
    }
    if (mediaQuery.includes('max-device-width')) {
        const maxDeviceWidth = parseFloat(mediaQuery.match(/max-device-width\s*:\s*(\d+(?:\.\d+)?)px/)?.[1] || '0');
        return deviceSize <= maxDeviceWidth;
    }
    if (mediaQuery.includes('min-device-width')) {
        const minDeviceWidth = parseFloat(mediaQuery.match(/min-device-width\s*:\s*(\d+(?:\.\d+)?)px/)?.[1] || '0');
    ...
    if (mediaQuery.includes('resolution') || mediaQuery.includes('-webkit-device-pixel-ratio')) {
...
export const convertToPixels = (value, unit, deviceSize)=>{
    switch(unit){
        // Viewport units
        case 'vw':
