import { hookable, hookBefore, getHooks, clearHooks, lockHooks } from '../../hookable.js';
...
describe('hookBefore', ()=>{
    ...
    it('It should add before hook to the registry with priority', ()=>{
        ...
        const { beforeHooks } = getHooks();
        expect(JSON.stringify(beforeHooks.get('test2'))).toEqual(JSON.stringify([
            {
...
describe('hookable', ()=>{
    ...
    it('It should call the before hook in correct order async', ()=>{
        ...
        const beforeCallback1 = jest.fn(async ()=>new Promise((resolve)=>resolve(data.push(1))));
        const beforeCallback2 = jest.fn(async ()=>new Promise((resolve)=>resolve(data.push(2))));
        const beforeCallback3 = jest.fn(async ()=>new Promise((resolve)=>resolve(data.push(3))));
        hookBefore('mockConstructor', beforeCallback1);
        hookBefore('mockConstructor', beforeCallback2);
        hookBefore('mockConstructor', beforeCallback3, 1);
        const hookedFunc = hookable(test);
    ...
    it('It should call the before hook in correct order async', async ()=>{
        clearHooks();
        const data = [];
        const test = jest.fn(async ()=>new Promise((resolve)=>{
                data.push(0);
                setTimeout(()=>{
                    data.push(4);
                    resolve();
