import { getValue, getValueSync, addProcessor, getProcessors } from '../../registry.js';
...
describe('registry', ()=>{
    ...
    it('It should add processor to the registry with priority', ()=>{
        ...
        expect(JSON.stringify(processors)).toEqual(JSON.stringify([
            {
    ...
    it('It should skip the processors if the init value and the context are identical', async ()=>{
        ...
        const value = await getValue('test4', 1, {
            a: 1
        });
        expect(value).toEqual(3);
        expect(callback3).toHaveBeenCalled();
        expect(callback2).toHaveBeenCalled();
        expect(callback1).toHaveBeenCalled();
        const value2 = await getValue('test4', 1, {
            a: 1
        });
    ...
    it('It should overwrite the init value and the context if the init value and the context are not identical', async ()=>{
        ...
        const value = await getValue('test5', 1, {
            a: 1
        });
        expect(value).toEqual(2);
        expect(callback).toHaveBeenCalled();
        const valueAgain = await getValue('test5', 1, {
            a: 1
        });
        expect(valueAgain).toEqual(2);
        expect(callback).toHaveBeenCalledTimes(1);
        const value2 = await getValue('test5', 2, {
            a: 2
        });
        expect(value2).toEqual(3);
        expect(callback).toHaveBeenCalledTimes(2);
        const value3 = await getValue('test5', 1, {
            a: 1
        });
