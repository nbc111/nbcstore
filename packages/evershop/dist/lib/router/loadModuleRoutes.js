...
export const loadModuleRoutes = (modulePath)=>{
    ...
    if (existsSync(path.resolve(modulePath, 'pages', 'admin'))) {
        ...
        adminControllerRoutes.forEach((route)=>{
            registerAdminRoute(route.id, route.method, route.path, route.name, route.isApi, route.folder);
        });
    }
    if (existsSync(path.resolve(modulePath, 'pages', 'frontStore'))) {
        ...
        frontStoreControllerRoutes.forEach((route)=>{
            registerFrontStoreRoute(route.id, route.method, route.path, route.name, route.isApi, route.folder);
        });
    }
    ...
    if (existsSync(path.resolve(modulePath, 'api'))) {
        ...
        routes.forEach((route)=>{
            registerFrontStoreRoute(route.id, route.method, route.path, route.name, route.isApi, route.folder, route.payloadSchema, route.access);
        });
    }
};
