...
export function parseGraphqlByFile(module) {
    ...
    /** Process query */ // Regex matching export const query = `...` or export const query = "" or export const query = ''
    ...
    if (queryMatch) {
        ...
        queryBody = queryBody.replace(/getWidgetSetting\(([^)]+)\)/g, (match, p1)=>{
            const base64 = Buffer.from(p1).toString('base64');
    ...
    /** Process fragments */ // Regex matching export const query = `...` or export const query = "" or export const query = ''
    ...
    const uniqueFragmentNames = [
        ...new Set(fragmentConsumptions.map((consumption)=>consumption.replace(/\.\.\.([ ]+)?/, '')))
    ];
    ...
    /** Processing variables */ // Regex matching export const variables = `{ ... }`
    ...
    if (variablesMatch) {
        ...
        variablesBody = variablesBody.replace(/getWidgetSetting\(([^)]*)\)/g, (match, p1)=>{
            const base64 = Buffer.from(p1).toString('base64');
            return `"getWidgetSetting_${base64}"`;
        });
        try {
