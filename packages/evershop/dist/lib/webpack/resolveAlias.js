...
function getAllFilesInFolder(folderPath) {
    ...
    for (const item of contents){
        ...
        if (fs.lstatSync(itemPath).isDirectory()) {
        ...
        } else if (/.js$/.test(item) && /^[A-Z]/.test(item[0]) || /\.(css|scss)$/i.test(item)) {
            ...
            const alias = path.join('@components', ...pathParts.slice(componentsIndex + 1)).replace('.js', '').replace('.scss', '').replace('.css', '');
...
export function resolveAlias(extensions = [], themePath = null) {
    ...
    extensions.forEach((extension)=>{
        const extensionFiles = getAllFilesInFolder(path.resolve(extension.path, 'components'));
        resolves = {
            ...extensionFiles,
            ...resolves
        };
