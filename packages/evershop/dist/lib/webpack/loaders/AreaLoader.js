...
const buildWidgetComponentsPerRoute = (route, widgets, imports)=>{
    ...
    widgets.forEach((widget)=>{
        const componentPath = route.isAdmin ? widget.settingComponent : widget.component;
...
export default function AreaLoader(c) {
    ...
    const routes = getRoutes().filter((route)=>route.isApi === false && route.isAdmin === isAdmin);
    ...
    try {
        Object.keys(components).forEach((routeId)=>{
            ...
            Object.assign(allRootComponents[routeId], {
                '*': widgetComponents
            });
        });
    } catch (e) {
