...
export default async function TailwindLoader(c) {
    ...
    mergedTailwindConfig.content = [
        ...
        join(CONSTANTS.ROOTPATH, 'node_modules', '@evershop', 'evershop', 'dist', '**', '[A-Z]*.js'),
        ...enabledExtensions.map((extension)=>join(extension.path, 'dist', '**', '[A-Z]*.js'))
    ];
    const theme = getEnabledTheme();
    if (theme) {
        mergedTailwindConfig.content.push(join(theme.path, 'dist', '**', '[A-Z]*.js'));
    }
    return postcss([
