...
export function createConfigClient(isAdmin = false) {
    ...
    config.entry = ()=>{
        const entry = [
            path.resolve(CONSTANTS.MODULESPATH, '../components/common/react/client/Index.js'),
            isAdmin ? `webpack-hot-middleware/client?path=/__webpack_hmr_admin&reload=true&overlay=true` : `webpack-hot-middleware/client?path=/__webpack_hmr_frontstore&reload=true&overlay=true`
        ];
        return entry;
    };
    config.watchOptions = {
    ...
    const nodeModuleExtensions = extensions.filter((ext)=>ext.path && ext.path.includes('node_modules')).map((ext)=>{
        // Extract package name from path (e.g., @vendor/package or package-name)
        const match = ext.path.match(/node_modules[\\/](@[^/\\]+[\\/][^/\\]+|[^/\\]+)/);
        return match ? match[1].replace(/\\/g, '[\\\\/]') : null;
    }).filter(Boolean).join('|');
    const managedPathsPattern = nodeModuleExtensions ? `^(.+?[\\\\/]node_modules[\\\\/](?!(@evershop[\\\\/]evershop|${nodeModuleExtensions}))(@.+?[\\\\/])?.+?)[\\\\/]` : `^(.+?[\\\\/]node_modules[\\\\/](?!(@evershop[\\\\/]evershop))(@.+?[\\\\/])?.+?)[\\\\/]`;
    config.snapshot = {
        managedPaths: [
            new RegExp(managedPathsPattern)
        ]
    };
    return config;
}
