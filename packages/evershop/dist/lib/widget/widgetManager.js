 ...
 */ function isValidJsFilePath(filePath) {
 ...
 */ function isComponentNameUppercase(filePath) {
    if (typeof filePath !== 'string' || filePath.trim() === '') {
        return false;
    }
    const baseName = path.parse(filePath).name;
    return baseName.length > 0 && baseName[0] === baseName[0].toUpperCase() && !!baseName[0].match(/[A-Z]/);
}
/**
 ...
 */ function isValidType(type) {
    return typeof type === 'string' && type.trim() !== '' && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(type);
}
class WidgetManager {
    /**
   * @private
   ...
   */ _ensureMutable() {
        if (this._isFrozen) {
            throw new Error('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        }
    }
    /**
   ...
   */ registerWidget(widget) {
        ...
        if (!widget || isValidType(widget.type) === false) {
            throw new Error('Cannot register widget. Widget object must have a valid "type" property.');
        }
        const widgetType = widget.type;
        if (this.widgets.has(widgetType)) {
            warning(`Widget with type "${widgetType}" is already registered. Skipping registration.`);
            return false;
        }
        if (!isValidJsFilePath(widget.settingComponent)) {
            throw new Error(`Cannot register widget "${widgetType}". Invalid or unresolvable settingComponent path: "${widget.settingComponent}". Please ensure it's a valid path to an existing JS file.`);
        }
        if (!isComponentNameUppercase(widget.settingComponent)) {
            throw new Error(`Cannot register widget "${widgetType}". Setting component filename "${path.parse(widget.settingComponent).name}" must start with an uppercase letter.`);
        }
        if (!isValidJsFilePath(widget.component)) {
            throw new Error(`Cannot register widget "${widgetType}". Invalid or unresolvable component path: "${widget.component}". Please ensure it's a valid path to an existing JS file.`);
        }
        if (!isComponentNameUppercase(widget.component)) {
            throw new Error(`Cannot register widget "${widgetType}". Main component filename "${path.parse(widget.component).name}" must start with an uppercase letter.`);
        }
   ...
   */ updateWidget(widgetType, updates) {
        this._ensureMutable();
        if (!updates || typeof updates !== 'object' || Object.keys(updates).length === 0) {
            throw new Error(`Cannot update widget "${widgetType}". No updates provided or updates object is invalid.`);
        }
        const typeToUpdate = widgetType;
        const existingWidget = this.widgets.get(typeToUpdate);
        if (!existingWidget) {
            throw new Error(`Cannot update widget "${typeToUpdate}". Widget not found.`);
        }
        if (updates.settingComponent !== undefined) {
            if (!isValidJsFilePath(updates.settingComponent)) {
                throw new Error(`Cannot update widget "${typeToUpdate}". Invalid or unresolvable new settingComponent path: "${updates.settingComponent}". Please ensure it's a valid path to an existing JS file.`);
            } else if (!isComponentNameUppercase(updates.settingComponent)) {
                throw new Error(`Cannot update widget "${typeToUpdate}". New setting component filename "${path.parse(updates.settingComponent).name}" must start with an uppercase letter.`);
            } else {
        ...
        if (updates.component !== undefined) {
            if (!isValidJsFilePath(updates.component)) {
                throw new Error(`Error: Cannot update widget "${typeToUpdate}". Invalid or unresolvable new component path: "${updates.component}". Please ensure it's a valid path to an existing JS file.`);
            } else if (!isComponentNameUppercase(updates.component)) {
                throw new Error(`Error: Cannot update widget "${typeToUpdate}". New main component filename "${path.parse(updates.component).name}" must start with an uppercase letter.`);
            } else {
                existingWidget.component = updates.component;
            }
        }
        for(const key in updates){
            if (Object.prototype.hasOwnProperty.call(updates, key) && key !== 'settingComponent' && key !== 'component') {
                existingWidget[key] = updates[key];
            }
        }
        return true;
    }
   ...
   */ getAllWidgets() {
        ...
        return Array.from(this.widgets.values()).map((widget)=>Object.freeze({
                ...widget
            }));
    }
    /**
 ...
 */ export function getEnabledWidgets() {
    const allWidgets = widgetManager.getAllWidgets();
    return allWidgets.filter((widget)=>widget.enabled).map((widget)=>{
        return {
            ...widget,
            settingComponentKey: generateComponentKey(widget.settingComponent),
            componentKey: generateComponentKey(widget.component)
 ...
 */ export function updateWidget(widgetType, updates) {
 ...
 */ export function removeWidget(widgetName) {
 ...
 */ export function hasWidget(widgetType) {
