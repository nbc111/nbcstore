...
jest.unstable_mockModule('fs', ()=>({
        existsSync: jest.fn((path)=>{
            if (path.includes('InvalidComponent.js') || path.includes('InvalidSetting.js')) {
...
describe('Widget Manager Module', ()=>{
    ...
    describe('Mutation after getAllWidgets()', ()=>{
        it('should throw an error if either component or settingComponent is not a string', async ()=>{
            ...
            expect(()=>widgetModule.registerWidget(invalidWidget)).toThrow('Invalid or unresolvable');
        });
        it('should throw an error if either component or settingComponent is unresolvable path', async ()=>{
            ...
            const invalidWidget = {
                ...
                type: 'InvalidType',
                description: 'This widget has invalid components',
                settingComponent: 'InvalidSetting.js',
                component: 'InvalidComponent.js',
                enabled: true,
                defaultSettings: {
                    theme: 'dark'
                }
        ...
        it('should throw an error if registerWidget is called after getAllWidgets', async ()=>{
            ...
            expect(()=>widgetModule.registerWidget(widget)).toThrow('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        });
        it('should throw an error if updateWidget is called after getAllWidgets', async ()=>{
            ...
            widgetModule.getAllWidgets(); // This freezes the manager
            expect(()=>widgetModule.updateWidget('ExistingWidget', {
                    description: 'Updated'
                })).toThrow('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        });
        it('should throw an error if removeWidget is called after getAllWidgets', async ()=>{
            ...
            widgetModule.getAllWidgets(); // This freezes the manager
            expect(()=>widgetModule.removeWidget('WidgetToRemove')).toThrow('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        });
        ...
        it('should allow to updateWidget', async ()=>{
            ...
            expect(widgetModule.getWidget('UpdateWidget').component).toEqual('UpdatedComponent.js');
            expect(widgetModule.getWidget('UpdateWidget').description).toEqual('Updated Description');
        });
        it('should throw error if trying to updateWidget with non-existing widget', async ()=>{
            const widgetModule = await import('../../widgetManager.js');
            expect(()=>widgetModule.updateWidget('NonExistingWidget', {
                    description: 'Test'
        ...
        it('should thrown an error if trying to update widget after calling getAllWidgets', async ()=>{
            ...
            widgetModule.getAllWidgets(); // This freezes the manager
            expect(()=>widgetModule.updateWidget('WidgetToUpdate', {
