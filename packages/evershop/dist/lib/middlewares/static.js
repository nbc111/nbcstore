 ...
 */ const isValidFile = async (fullPath)=>{
    try {
...
export default (async (request, response, next)=>{
    ...
    if (await pathExists(buildPath) && await isValidFile(buildPath)) {
        return staticMiddleware(join(CONSTANTS.ROOTPATH, '.evershop/build'), staticMiddlewareOptions)(request, response, next);
    }
    // Check media path
    const mediaPath = join(CONSTANTS.MEDIAPATH, path);
    if (await pathExists(mediaPath) && await isValidFile(mediaPath)) {
        return staticMiddleware(CONSTANTS.MEDIAPATH, staticMiddlewareOptions)(request, response, next);
    }
    // Check public path
    const publicPath = join(CONSTANTS.ROOTPATH, 'public', path);
    if (await pathExists(publicPath) && await isValidFile(publicPath)) {
        return staticMiddleware(join(CONSTANTS.ROOTPATH, 'public'), staticMiddlewareOptions)(request, response, next);
    }
