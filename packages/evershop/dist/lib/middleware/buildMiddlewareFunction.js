 ...
 */ export function buildMiddlewareFunction(id, path) {
    ...
    const isRoutedLevel = ![
        'all',
        'global'
    ].includes(path.split(sep).reverse()[1]);
    // Check if the middleware is an error handler.
    // TODO: fix me
    if (id === 'errorHandler' || id === 'apiErrorHandler') {
        return async (error, request, response, next)=>{
            const m = isDevelopmentMode() ? await import(`${pathToFileURL(path)}?t=${Date.now()}`) : await import(pathToFileURL(path));
        ...
        return async (request, response, next)=>{
            ...
            if (response.statusCode === 404 && isRoutedLevel) {
                ...
                try {
                    ...
                    if (!func) {
                        if (isProductionMode()) {
                            throw new Error(`Middleware ${id} is invalid. It should provide a function as default export.`);
                        } else {
                            func = ()=>{
                                debug(`Middleware ${id} is not implemented yet. Please implement it.`);
                            };
                        }
                    }
                    if (func.length === 3) {
