...
describe('test getMiddlewaresByRoute', ()=>{
    ...
    it('should contains middlewares for product route', ()=>{
        expect(productMiddleweres.findIndex((m)=>m.id === 'loadCategory')).not.toBe(-1);
    ...
    it('should contains middlewares for product route', ()=>{
        expect(productEditMiddleweres.findIndex((m)=>m.id === 'loadCategory')).not.toBe(-1);
        expect(productEditMiddleweres.findIndex((m)=>m.id === 'loadProductImage')).not.toBe(-1);
    });
    it('should contains admin level middleware', ()=>{
        expect(productEditMiddleweres.findIndex((m)=>m.id === 'adminTitle')).not.toBe(-1);
    });
    it('should contains application level middleware', ()=>{
