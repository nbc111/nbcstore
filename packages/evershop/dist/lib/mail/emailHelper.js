 ...
 */ export function validateSendEmailArguments(args) {
    ...
    if (typeof typedArgs.subject !== 'string' || typedArgs.subject.trim() === '') {
        throw new Error('"subject" field must be a non-empty string.');
    }
    if (typeof typedArgs.template !== 'string' || typedArgs.template.trim() === '') {
        throw new Error('"template" field must be a non-empty string.');
    }
    // Body is optional, but it must be a string if provided
    if (typedArgs.body !== undefined && (typeof typedArgs.body !== 'string' || typedArgs.body.trim() === '')) {
        throw new Error('"body" field must be a non-empty string if provided.');
    }
    // Validate optional fields if present
    if (typedArgs.template !== undefined && typeof typedArgs.template !== 'string') {
 ...
 */ function isValidEmailService(service) {
    return typeof service === 'object' && service !== null && 'sendEmail' in service && typeof service.sendEmail === 'function';
}
/**
 * Retrieves the registered email service from the registry.
 * @returns The email service object.
 */ export function getEmailService() {
    const emailService = getValueSync('emailService', undefined, {}, isValidEmailService);
    return emailService;
}
 ...
 */ export function registerEmailService(service) {
    if (!isValidEmailService(service)) {
        throw new Error('Invalid email service. It must be an object with a sendEmail method.');
    }
 ...
 */ export async function sendEmail(id, args) {
    const emailService = getEmailService();
    if (!emailService) {
        return Promise.reject(new Error('No email service registered to send emails.'));
    }
    const finalArgs = await getValue('emailArguments', args, {
        id
    });
    if (!finalArgs?.from) {
    ...
    if (!finalArgs.body) {
        const body = await buildEmailBodyFromTemplate(finalArgs.template, finalArgs.data || {});
        finalArgs.body = body;
    }
 ...
 */ export async function buildEmailBodyFromTemplate(template, data) {
    try {
