...
function buildContextData(request, response) {
    const pageMeta = getPageMetaInfo(request);
    const appConfig = getValueSync('appConfig', {
        tax: {
...
function renderProduction(request, response) {
    ...
    for(let i = 0; i < assets.css.length; i++){
        const cssFilePath = path.resolve(getRouteBuildPath(route), 'client', path.basename(assets.css[i]));
    ...
    import(pathToFileURL(serverIndexPath).toString()).then((module)=>{
        const source = processPreloadImages(module.default(request.currentRoute, assets.js, cssList, safeContextValue, langCode));
