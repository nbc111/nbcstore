 ...
 */ export async function seedProductImages(productId, images) {
    ...
    for(let i = 0; i < images.length; i++){
        ...
        try {
            ...
            if (imageData.url && imageData.url.startsWith('http')) {
                ...
                const subPath = `catalog/${Math.floor(Math.random() * (9999 - 1000)) + 1000}/${Math.floor(Math.random() * (9999 - 1000)) + 1000}`;
                ...
                try {
                    ...
                    const existingImage = await select().from('product_image').where('product_image_product_id', '=', productId).and('origin_image', '=', finalImageUrl).load(pool);
                    if (!existingImage) {
