/* eslint-disable no-console */ import './initEnvDev.js';
...
const { argv } = yargs(hideBin(process.argv)).option('attributes', {
    alias: 'a',
    ...
    description: 'Seed all demo data (attributes, categories, collections, products, widgets, pages)',
    type: 'boolean',
    default: false
}).check((argv)=>{
    if (!argv.attributes && !argv.categories && !argv.collections && !argv.products && !argv.widgets && !argv.pages && !argv.all) {
        throw new Error('Please specify at least one option: --attributes, --categories, --collections, --products, --widgets, --pages, or --all');
    }
    return true;
}).help();
async function seed() {
    const options = argv;
    let demoAttributeGroupId = null;
    try {
