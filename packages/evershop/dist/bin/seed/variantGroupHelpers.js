 ...
 */ export async function createVariantGroups(productsData, demoAttributeGroupId, colorAttributeId) {
    const variantGroupIds = new Map();
    ...
    for (const groupName of uniqueGroups){
        try {
            ...
            const result = await insert('variant_group').given({
                uuid: uuid,
            ...
            success(`✓ Created variant group: ${groupName} (ID: ${result.insertId}, UUID: ${uuid})`);
        } catch (e) {
 ...
 */ export async function resolveAttributeOptions(attributes) {
    const validAttributes = [];
    for (const attr of attributes){
        // Check the attribute type
        const attribute = await select().from('attribute').where('attribute_code', '=', attr.attribute_code).load(pool);
        if (attribute && (attribute.type === 'select' || attribute.type === 'multiselect')) {
            // Look up the option ID by option text
            const option = await select().from('attribute_option').where('attribute_id', '=', attribute.attribute_id).and('option_text', '=', attr.value).load(pool);
            if (option) {
                // Replace the text value with the option ID
                attr.value = option.attribute_option_id.toString();
                validAttributes.push(attr);
                info(`  → Resolved ${attr.attribute_code}: "${option.option_text}" → ID ${option.attribute_option_id}`);
            } else {
                error(`  ✗ Option "${attr.value}" not found for attribute "${attr.attribute_code}" - skipping this attribute`);
