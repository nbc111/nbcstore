 ...
 */ export async function seedPages() {
    try {
        ...
        for (const pageData of pagesData){
            ...
            const existing = await select().from('cms_page_description').where('url_key', '=', pageData.url_key).load(connection, false);
            if (existing) {
            ...
            const page = await insert('cms_page').given({
                status: pageData.status
            }).execute(connection, false);
            // Insert cms_page_description
            await insert('cms_page_description').given({
        ...
        success(`✓ CMS pages seeding complete: ${created} created, ${skipped} skipped`);
    } catch (e) {
