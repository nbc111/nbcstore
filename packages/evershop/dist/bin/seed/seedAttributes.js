 ...
 */ export async function seedAttributes(demoAttributeGroupId) {
    ...
    for (const attributeData of attributesData){
        try {
            ...
            const existingAttribute = await select().from('attribute').where('attribute_code', '=', attributeData.attribute_code).load(pool);
            if (existingAttribute) {
                info(`Attribute "${attributeData.attribute_name}" already exists, updating options...`);
                ...
                if (attributeData.options && Array.isArray(attributeData.options)) {
                    for (const optionData of attributeData.options){
                        ...
                        const existingOption = await select().from('attribute_option').where('attribute_id', '=', existingAttribute.attribute_id).and('option_text', '=', optionData.option_text).load(pool);
                        if (!existingOption) {
                ...
                const existingLink = await select().from('attribute_group_link').where('attribute_id', '=', existingAttribute.attribute_id).and('group_id', '=', demoAttributeGroupId).load(pool);
                if (!existingLink) {
            ...
            error(`Failed to create attribute ${attributeData.attribute_name}: ${e.message}`);
        }
    }
}
