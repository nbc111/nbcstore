...
import { downloadImage, getFilenameFromUrl, convertToMediaPath } from './imageDownloader.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
/**
 * Download slideshow images and update URLs
 */ async function downloadSlideshowImages(settings) {
        ...
        for (const slide of settings.slides){
            if (slide.image && slide.image.startsWith('http')) {
                try {
                    ...
                    // Create local path
                    const mediaDir = join(CONSTANTS.ROOTPATH, 'media', 'widgets', slideId);
 ...
 */ export async function seedWidgets() {
    try {
        ...
        const widgetsData = JSON.parse(readFileSync(widgetsPath, 'utf-8'));
        const connection = await getConnection();
        let created = 0;
        let skipped = 0;
        for (const widgetData of widgetsData){
            // Check if widget already exists (by name and type)
            const existing = await select().from('widget').where('name', '=', widgetData.name).and('type', '=', widgetData.type).load(connection, false);
            if (existing) {
        ...
        success(`✓ Widget seeding complete: ${created} created, ${skipped} skipped`);
    } catch (e) {
