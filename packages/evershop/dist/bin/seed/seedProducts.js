...
import { createVariantGroups, resolveAttributeOptions } from './variantGroupHelpers.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
/**
 * Seed products from JSON file
 */ export async function seedProducts(demoAttributeGroupId) {
    ...
    const colorAttribute = await select().from('attribute').where('attribute_code', '=', 'color').load(pool);
    const sizeAttribute = await select().from('attribute').where('attribute_code', '=', 'size').load(pool);
    if (!colorAttribute || !sizeAttribute) {
        error('Color and Size attributes must be seeded first. Run: npm run seed -- --attributes');
        return;
    }
    // Create variant groups
    const variantGroupIds = await createVariantGroups(productsData, demoAttributeGroupId, colorAttribute.attribute_id);
    // Seed products
    info('\nSeeding products...');
    for (const productData of productsData){
        try {
            // Check if product already exists
            const existingProduct = await select().from('product').where('sku', '=', productData.sku).load(pool);
            if (existingProduct) {
                info(`Product "${productData.name}" (${productData.sku}) already exists, skipping...`);
                continue;
            }
            ...
            if (productData.category) {
                ...
                const categoryQuery = select('category.category_id').from('category_description');
                categoryQuery.leftJoin('category').on('category.category_id', '=', 'category_description.category_description_category_id');
                categoryQuery.where('category_description.url_key', '=', categoryUrlKey);
                const category = await categoryQuery.load(pool);
                if (category && category.category_id) {
                    productData.category_id = category.category_id;
                } else {
                    warning(`  ⚠️  Category "${categoryUrlKey}" not found, product will have no category`);
                }
            ...
            if (variantGroup && variantGroupIds.has(variantGroup)) {
                ...
                info(`  → Assigning to variant group: ${variantGroup} (ID: ${productData.variant_group_id})`);
            }
            // Convert attribute values to option IDs for select type attributes
            if (productData.attributes && Array.isArray(productData.attributes)) {
                productData.attributes = await resolveAttributeOptions(productData.attributes);
            }
            ...
            if (collections && Array.isArray(collections)) {
                for (const collectionCode of collections){
                    const collection = await select().from('collection').where('code', '=', collectionCode).load(pool);
                    if (collection) {
                        await insert('product_collection').given({
