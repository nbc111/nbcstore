 ...
 */ export async function buildEntry(routes, clientOnly = false) {
    ...
    await Promise.all(routes.map(async (route)=>{
        ...
        /** Build layout and query */ const areas = {};
      ...
      import {${route.isAdmin ? 'HydrateAdmin' : 'HydrateFrontStore'}} from '@evershop/evershop/components/common';
      `;
        areas['*'] = areas['*'] || {};
        widgets.forEach((widget)=>{
            const url = route.isAdmin ? pathToFileURL(widget.settingComponent).toString() : pathToFileURL(widget.component).toString();
            const id = generateComponentKey(route.isAdmin ? `admin_widget_${widget.type}` : `widget_${widget.type}`);
            imports.push(`import ${id} from '${url}';`);
            areas['*'][id] = {
                id,
        ...
        ${route.isAdmin ? 'React.createElement(HydrateAdmin, null)' : 'React.createElement(HydrateFrontStore, null)'},
        document.getElementById('app')
      );`;
        if (!fs.existsSync(path.resolve(subPath, 'client'))) {
            await mkdir(path.resolve(subPath, 'client'), {
                recursive: true
            });
        }
        await writeFile(path.resolve(subPath, 'client', 'entry.js'), contentClient);
        if (!clientOnly) {
            /** Build query */ const query = `${JSON.stringify(parseGraphql(components))}`;
            ...
            if (!fs.existsSync(path.resolve(subPath, 'server'))) {
                await mkdir(path.resolve(subPath, 'server'), {
                    recursive: true
                });
            }
