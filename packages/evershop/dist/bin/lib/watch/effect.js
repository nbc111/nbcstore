...
export function detectEffect(event) {
    ...
    if (isRestartRequired(event)) {
        ...
        if (event.type === 'create') {
            if (minimatch(event.path.toString(), '**/pages/+(admin|frontStore)/[A-Z]*.+(jsx|tsx)')) {
    ...
    } else if (minimatch(event.path.toString(), '**/+(api|admin|frontStore)/*/[a-z[]*.+(js|ts)')) {
        const routeFolder = basename(dirname(event.path.toString()));
        if (!isValidRouteFolder(routeFolder)) {
            return 'unknown'; // Not a valid route folder, skip
        }
    ...
    } else if (minimatch(event.path.toString(), '**/pages/frontStore/*/route.json')) {
        const routeFolder = basename(dirname(event.path.toString()));
        if (!isValidRouteFolder(routeFolder)) {
            return 'unknown'; // Not a valid route folder, skip
        }
    ...
    } else if (minimatch(event.path.toString(), '**/*/*.graphql') || minimatch(event.path.toString(), '**/*/*.resolvers.+(ts|js)')) {
    ...
    } else if (event.path && jobs.some((job)=>job.resolve === event.path.toString().replace('src', 'dist').replace(/\.ts$/, '.js'))) {
        return 'restart_cronjob';
    } else if (isSrc(event.path.toString())) {
        const distPath = event.path.toString().replace('src', 'dist').replace(/\.ts$/, '.js');
        if (has(distPath)) {
...
export function applyEffects(events, app) {
    ...
    if (events.some((e)=>e.effect && ![
            'unknown',
            'restart_cronjob',
            'restart_event'
        ].includes(e.effect) && !e.effect.includes('component'))) {
