...
import { applyEffects, detectEffect } from './effect.js';
import { isDist } from './isDist.js';
import { isSrc } from './isSrc.js';
import { restartProcess } from './processors/restart.js';
export async function watchHandler(events, app) {
    if (events.length === 2 && events.some((e)=>e.type === 'delete') && events.some((e)=>e.type === 'create')) {
        // Likely a rename
                ...
                try {
                    ...
                    if (stats.isDirectory()) {
                        ...
                        for (const file of files){
                            const filePath = path.resolve(event.path, file);
                            events.push({
                                path: filePath,
                                type: 'create'
                            });
    ...
    for (const event of events){
        ...
        if (isSrc(event.path)) {
            const distPath = event.path.toString().replace('src', 'dist').replace(/\.ts$/, '.js').replace(/\.tsx$/, '.js').replace(/\.jsx$/, '.js'); // Ensure the path ends with .js
