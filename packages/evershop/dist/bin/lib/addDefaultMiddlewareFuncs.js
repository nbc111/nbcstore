...
export function addDefaultMiddlewareFuncs(app) {
    app.use((request, response, next)=>{
        ...
        response.on('finish', ()=>{
            ...
            response.debugMiddlewares.forEach((m)=>{
                message += m.time ? `-> Middleware ${m.id} - ${m.time} ms\n` : `-> Middleware ${m.id}\n`;
            });
            // Skip logging if the request is for static files
            if (request.currentRoute?.id === 'staticAsset' || request.currentRoute?.id === 'adminStaticAsset') {
                return;
            }
            debug(message);
        });
    ...
    const sess = {
        store: process.env.NODE_ENV === 'test' ? undefined : new (sessionStorage(session))({
            pool
        }),
        secret: cookieSecret,
        cookie: {
    ...
    app.use((request, response, next)=>{
        ...
        if (matchedRoutes.length > 1) {
            warning(`Multiple routes matched for ${requestPath}. Please check your routes: ${matchedRoutes.map((r)=>r.id).join(', ')}. Route ${matchedRoutes[0].id} will be used.`);
        }
    ...
    app.use(async (request, response, next)=>{
        ...
        const rewriteRule = await select().from('url_rewrite').where('request_path', '=', `/${path}`).load(pool);
        if (rewriteRule) {
            // Find the route
            const routes = getRoutes();
    ...
    if (isDevelopmentMode()) {
        ...
        app.use((request, response, next)=>{
            if (!request.path.startsWith('/backend/') && !request.path.startsWith('/__webpack_hmr_admin')) {
    ...
    /** 404 Not Found handle */ app.use((request, response, next)=>{
