...
(async ()=>{
    ...
    spinner.start();
    /** Initilizing routes */ modules.forEach((module)=>{
        try {
            // Load routes
    ...
    /** Initializing components */ modules.forEach((module)=>{
        try {
            // Load components
    ...
    /** Get list of routes */ const routes = getRoutes();
    /** Collect all 'controller' routes */ const controllers = routes.filter((r)=>r.isApi === false);
    /** Clean up the build directory */ if (existsSync(CONSTANTS.BUILDPATH)) {
        rmSync(CONSTANTS.BUILDPATH, {
            recursive: true
        });
    }
    /** Create components.js file for each route */ await createComponents(controllers);
    /** Create the webpack complier object */ const compiler = webpack(createConfig(true, controllers));
    /** Run the build */ await new Promise((resolve, reject)=>{
