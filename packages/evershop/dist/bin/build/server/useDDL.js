...
webpackVendorPromise.then(async ()=>{
    controllers.forEach((route)=>{
        const buildFunc = async function() {
            ...
            let content = `var components = module.exports = exports = ${inspect(components, {
                depth: 5
            }).replace(/'---/g, '').replace(/---'/g, '')}`;
            content += '\r\n';
            await mkdir(path.resolve(CONSTANTS.ROOTPATH, './.evershop/build', buildPath), {
                recursive: true
            });
            ...
            entry[name] = [
                ...
                path.resolve(CONSTANTS.LIBPATH, '../components/common/react/server', 'render.js')
            ];
            const compiler = webpack({
                ...
                entry,
                output: {
                    path: path.resolve(CONSTANTS.ROOTPATH, './.evershop/build', buildPath, 'server'),
                ...
                plugins: [
                    new webpack.DllReferencePlugin({
                        manifest: path.resolve(CONSTANTS.ROOTPATH, './.evershop/build/vendor-manifest.json')
                    })
                ]
            });
