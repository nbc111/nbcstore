#!/usr/bin/env node
/* eslint-disable no-console */ import { exec } from 'child_process';
...
async function selectTheme() {
    ...
    try {
        ...
        themeNames = files.filter((dirent)=>dirent.isDirectory()).map((dirent)=>dirent.name);
...
async function updateConfig(theme) {
    ...
    try {
        ...
        console.log(boxen(kleur.green(`Theme updated to "${theme}" in config/default.json`), {
            padding: 1,
            borderColor: 'green'
        }));
    } catch (err) {
...
async function activateTheme() {
    ...
    if (shouldBuild) {
        ...
        console.log(kleur.yellow('Remember to run "npm run build" later to apply changes.'));
    }
}
