...
async function createTheme() {
    ...
    if (!/^[A-Za-z0-9_-]+$/.test(name)) {
        console.error(kleur.red('Invalid theme name. Use only letters, numbers, dashes or underscores.'));
        process.exit(1);
    }
    ...
    try {
        ...
        const tsconfigContent = {
            compilerOptions: {
                ...
                rootDir: 'src',
                paths: {
                    '@components/*': await isRealDirectory(path.join(process.cwd(), 'node_modules', '@evershop', 'evershop')) ? [
                        './src/components/*',
