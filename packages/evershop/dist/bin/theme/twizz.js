...
async function findAllDependencies(filePath, visited = new Set(), baseDir) {
        ...
        for (const importPath of relativeImports){
            ...
            try {
                ...
                if (resolvedPath.startsWith(baseDir)) {
                    ...
                    const nestedDeps = await findAllDependencies(resolvedPath, visited, baseDir);
                ...
                for (const ext of extensions){
                    ...
                    try {
                        ...
                        if (pathWithExt.startsWith(baseDir)) {
                            ...
                            const nestedDeps = await findAllDependencies(pathWithExt, visited, baseDir);
                            dependencies.push(...nestedDeps);
                            found = true;
                            break;
                        }
                    ...
                    for (const ext of extensions){
                        ...
                        try {
                            ...
                            if (indexPath.startsWith(baseDir)) {
                                ...
                                const nestedDeps = await findAllDependencies(indexPath, visited, baseDir);
                                dependencies.push(...nestedDeps);
                                break;
                            }
                        } catch  {
...
async function scanModulesFrontStore() {
    const evershopDir = path.join(process.cwd(), 'node_modules', '@evershop', 'evershop');
    const modulesDir = await isRealDirectory(evershopDir) ? path.join(evershopDir, 'src', 'modules') : path.join(process.cwd(), 'packages', 'evershop', 'src', 'modules');
    let results = [];
    try {
        ...
        for (const mod of modules){
            if (mod.isDirectory()) {
                const frontStoreDir = path.join(modulesDir, mod.name, 'pages', 'frontStore');
...
async function getOverrideCandidates() {
    ...
    if (await isRealDirectory(evershopDir)) {
        ...
        commonDir = path.join(process.cwd(), 'packages', 'evershop', 'src', 'components', 'common');
        frontStoreDir = path.join(process.cwd(), 'packages', 'evershop', 'src', 'components', 'frontStore');
    }
...
function getCurrentTheme() {
    ...
    if (theme) {
        ...
        console.error(kleur.red('No theme set in config/system.theme. Please set a theme before creating overrides.'));
        process.exit(1);
    }
}
// Given an original file path and current theme, determine the destination override file path
function getDestinationPath(originalPath, theme) {
    ...
    if (componentsIdx !== -1) {
        ...
        const relativePath = originalPath.substring(originalPath.indexOf('components'));
        ...
        if (markerIdx !== -1) {
            const relativePath = originalPath.substring(markerIdx + frontStoreMarker.length);
            // Ensure leading slash is removed
            const cleanedRelative = relativePath.replace(new RegExp(`^(\\${path.sep}|/)`), '');
...
async function createOverrideFile() {
    ...
    const relativeCandidates = candidates.map((filePath)=>path.relative(process.cwd(), filePath));
    ...
    const evershopDir = path.join(process.cwd(), 'node_modules', '@evershop', 'evershop');
    const baseDir = await isRealDirectory(evershopDir) ? path.join(evershopDir, 'src') : path.join(process.cwd(), 'packages', 'evershop', 'src');
    // Find all dependencies of the selected file
    console.log(kleur.yellow('Analyzing dependencies...'));
    const dependencies = await findAllDependencies(selectedFile, new Set(), baseDir);
    const allFiles = [
        selectedFile,
        ...dependencies
    ];
    ...
    if (copiedFiles.length > 0) {
        console.log(boxen(kleur.green(`Successfully created ${copiedFiles.length} override file(s):\n`) + copiedFiles.map((file)=>kleur.white(`• ${file}`)).join('\n'), {
            padding: 1,
            borderColor: 'green'
        }));
    } else {
