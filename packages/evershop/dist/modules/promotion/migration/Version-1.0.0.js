...
export default (async (connection)=>{
    await execute(connection, `CREATE TABLE "coupon" (
  ...
  CONSTRAINT "COUPON_UNIQUE" UNIQUE ("coupon")
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION set_coupon_used_time()
        RETURNS TRIGGER 
        LANGUAGE PLPGSQL
        AS
      $$
      ...
      END;
      $$;`);
    await execute(connection, `CREATE TRIGGER "TRIGGER_UPDATE_COUPON_USED_TIME_AFTER_CREATE_ORDER" AFTER INSERT ON "order"
     FOR EACH ROW 
