...
export default {
    ...
    Coupon: {
            ...
            if (!userCondition) {
                ...
                return {
                    ...
                    emails: Array.isArray(userCondition.emails) // For backward compatibility
                     ? userCondition.emails : userCondition.emails.split(',').map((email)=>email.trim())
                };
            }
        },
        buyxGety: ({ buyxGety })=>{
        ...
        editUrl: ({ uuid })=>buildUrl('couponEdit', {
                id: uuid
            }),
        updateApi: (coupon)=>buildUrl('updateCoupon', {
                id: coupon.uuid
            }),
        deleteApi: (coupon)=>buildUrl('deleteCoupon', {
                id: coupon.uuid
            })
    }
};
