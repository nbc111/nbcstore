...
export function registerDefaultCalculators() {
    return [
        async function percentageDiscountToEntireOrderCalculator(cart, coupon) {
            ...
            const cartSubTotal = priceIncludingTax ? cart.getData('sub_total_incl_tax') : cart.getData('sub_total');
            const cartDiscountAmount = toPrice(discountPercent * cartSubTotal / 100);
            ...
            items.forEach((item, index)=>{
                ...
                if (index === items.length - 1) {
                    ...
                    const lineTotal = priceIncludingTax ? item.getData('line_total_incl_tax') : item.getData('line_total');
                    sharedDiscount = toPrice(lineTotal * cartDiscountAmount / cartSubTotal, 0);
                }
                if (discounts[item.getId()] || discounts[item.getId()] !== sharedDiscount) {
        ...
        async function fixedDiscountToEntireOrderCalculator(cart, coupon) {
            ...
            const cartSubTotal = priceIncludingTax ? cart.getData('sub_total_incl_tax') : cart.getData('sub_total');
            cartDiscountAmount = cartSubTotal > cartDiscountAmount ? cartDiscountAmount : cartSubTotal;
            let distributedAmount = 0;
            const discounts = {};
            const items = cart.getItems();
            items.forEach((item, index)=>{
        ...
        async function discountToSpecificProductsCalculator(cart, coupon) {
            ...
            const collections = await select().from('product_collection').where('product_id', 'IN', items.map((item)=>item.getData('product_id'))).execute(pool);
            items.forEach((item)=>{
                ...
                targetProducts.forEach((targetProduct)=>{
                    ...
                    if (key === 'collection') {
                        ...
                        if (operator === 'IN') {
                            flag = collections.some((collection)=>requiredCollectionIDs.includes(collection.collection_id) && collection.product_id === item.getData('product_id'));
                        } else {
                            flag = !collections.some((collection)=>requiredCollectionIDs.includes(collection.collection_id) && collection.product_id === item.getData('product_id'));
                        }
                    }
                ...
                if (coupon.discount_type === 'fixed_discount_to_specific_products') {
                    discountAmount = Math.min(discountAmount, priceIncludingTax ? item.getData('final_price_incl_tax') : item.getData('final_price'));
                    discounts[item.getId()] = Math.min(item.getData('qty'), maxQty) * discountAmount;
                } else {
                    const discountPercent = Math.min(discountAmount, 100);
                    discounts[item.getId()] = toPrice(discountPercent * (priceIncludingTax ? item.getData('final_price_incl_tax') : item.getData('final_price')) / 100 * Math.min(item.getData('qty'), maxQty));
                }
            });
            await Promise.all(items.map(async (item)=>{
                await item.setData('discount_amount', discounts[item.getId()] || 0);
            ...
            configs.forEach((row)=>{
                ...
                for(let i = 0; i < items.length; i += 1){
                    ...
                    if (item.getData('product_sku') === sku.trim() && item.getData('qty') >= buyQty + getQty) {
                        const discountPerUnit = toPrice(discount * (priceIncludingTax ? item.getData('final_price_incl_tax') : item.getData('final_price')) / 100);
                        ...
                        if (discounts[item.getId()] || discounts[item.getId()] !== discountAmount) {
