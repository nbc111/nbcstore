...
export function registerCartItemPromotionFields(fields) {
    const newFields = fields.concat(/** Adding fields to the Cart Item object */ [
        {
            key: 'discount_amount',
            resolvers: [
            ...
            resolvers: [
                async function resolver() {
                    ...
                    if (!priceIncludingTax) {
                        return this.getData('line_total') - this.getData('discount_amount');
                    } else {
                        return this.getData('line_total_incl_tax') - this.getData('discount_amount') - this.getData('tax_amount');
                    }
                }
            ],
            dependencies: [
        ...
        },
        {
            ...
            resolvers: [
                async function resolver() {
                    const priceIncludingTax = getConfig('pricing.tax.price_including_tax', false);
                    if (!priceIncludingTax) {
                        return toPrice(this.getData('line_total') - this.getData('discount_amount') + this.getData('tax_amount'));
                    } else {
                        return toPrice(this.getData('line_total_incl_tax') - this.getData('discount_amount'));
                    }
                }
            ],
            dependencies: [
