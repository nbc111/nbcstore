...
export function registerDefaultValidators() {
    return [
        function generalValidator(cart, coupon) {
            ...
            const startDate = coupon.start_date ? DateTime.fromJSDate(coupon.start_date, {
                zone: 'UTC'
            }) : today.minus({
                days: 1
            });
            const endDate = coupon.end_date ? DateTime.fromJSDate(coupon.end_date, {
                zone: 'UTC'
            }) : today.plus({
                days: 1
            });
        ...
        async function timeUsedValidator(cart, coupon) {
            if (coupon.max_uses_time_per_coupon && parseInt(coupon.used_time, 10) >= parseInt(coupon.max_uses_time_per_coupon, 10)) {
                return false;
            }
            if (coupon.max_uses_time_per_customer) {
                const customerId = cart.getData('customer_id');
                if (customerId) {
                    const flag = await select().from('customer_coupon_use').where('customer_id', '=', customerId).andWhere('coupon', '=', coupon.coupon).andWhere('used_time', '>=', parseInt(coupon.max_uses_time_per_customer, 10)).execute(pool);
                    if (flag) {
                        return false;
                    }
                }
        ...
        function subTotalValidator(cart, coupon) {
            ...
            const conditions = coupon.condition;
            const minimumSubTotal = !Number.isNaN(parseFloat(conditions.order_total)) ? parseFloat(conditions.order_total) : null;
        ...
        function minimumQtyValidator(cart, coupon) {
            const conditions = coupon.condition;
            const minimumQty = !Number.isNaN(parseInt(conditions.order_qty, 10)) ? parseInt(conditions.order_qty, 10) : null;
        ...
        async function requiredProductByCollectionValidator(cart, coupon) {
            ...
            for(let index = 0; index < requiredProducts.length; index += 1){
                ...
                if (condition.key !== 'collection') {
                    ...
                    const requiredCollectionIDs = value.map((v)=>parseInt(v.trim(), 10));
                    const productIds = items.map((item)=>item.getData('product_id'));
                    // Load the collections of all item
                    if (!collections) {
                        collections = await select().from('product_collection').where('product_id', 'IN', productIds).execute(pool);
                    }
                    if (operator === 'IN') {
                        items.forEach((item)=>{
                            const productId = item.getData('product_id');
                            const collectionIDs = collections.filter((collection)=>parseInt(collection.product_id, 10) === productId && requiredCollectionIDs.includes(parseInt(collection.collection_id, 10)));
        ...
        async function requiredProductByPriceValidator(cart, coupon) {
            ...
            for(let index = 0; index < requiredProducts.length; index += 1){
                ...
                if (condition.key !== 'price') {
                    ...
                    items.forEach((item)=>{
                        const price = priceIncludingTax ? item.getData('final_price_incl_tax') : item.getData('final_price');
        ...
        async function customerPurchasesAmountValidator(cart, coupon) {
            const conditions = coupon.user_condition;
            const purchasedAmount = conditions.purchased ? parseFloat(conditions.purchased.trim()) || null : null;
            ...
            const query = select();
            query.from('order').select('SUM(grand_total)', 'total').where('customer_id', '=', cart.getData('customer_id')).andWhere('payment_status', '=', 'paid');
