...
import pageDataSchema from './pageDataSchema.json' with {
    type: 'json'
};
...
async function updatePageData(uuid, data, connection) {
    ...
    const newPage = await update('cms_page').given(data).where('uuid', '=', uuid).execute(connection);
    Object.assign(page, newPage);
    let description = {};
    try {
        description = await update('cms_page_description').given(data).where('cms_page_description_cms_page_id', '=', page.cms_page_id).execute(connection);
    } catch (e) {
 ...
 */ async function updatePage(uuid, data, context) {
    ...
    try {
        ...
        // Insert page data
        const page = await hookable(updatePageData, {
            ...context,
            connection
        })(uuid, pageData, connection);
...
export function hookBeforeUpdatePageData(callback, priority = 10) {
    hookBefore('updatePageData', callback, priority);
}
export function hookAfterUpdatePageData(callback, priority = 10) {
    hookAfter('updatePageData', callback, priority);
}
export function hookBeforeUpdatePage(callback, priority = 10) {
    hookBefore('updatePage', callback, priority);
}
export function hookAfterUpdatePage(callback, priority = 10) {
