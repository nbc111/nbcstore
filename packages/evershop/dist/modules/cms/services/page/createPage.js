...
async function insertPageData(data, connection) {
    ...
    const description = await insert('cms_page_description').given(data).prime('cms_page_description_cms_page_id', page.insertId).execute(connection);
    return {
        ...description,
        ...page
    };
 ...
 */ const _createPage = async function createPage(data, context) {
    ...
    try {
        ...
        // Insert page data
        const page = await hookable(insertPageData, {
            ...context,
            connection
        })(pageData, connection);
...
export async function createPage(data, context) {
...
export function hookBeforeInsertPageData(callback, priority = 10) {
    hookBefore('insertPageData', callback, priority);
}
export function hookAfterInsertPageData(callback, priority = 10) {
    hookAfter('insertPageData', callback, priority);
}
export function hookBeforeCreatePage(callback, priority = 10) {
    hookBefore('createPage', callback, priority);
}
export function hookAfterCreatePage(callback, priority = 10) {
