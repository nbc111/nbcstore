...
describe('imageProcessor', ()=>{
    ...
    beforeAll(async ()=>{
        ...
        try {
            await fs.mkdir(path.join(ROOTPATH, 'media/cache'), {
                recursive: true
            });
        } catch (error) {
    ...
    describe('Basic functionality', ()=>{
        ...
        test('Should process transparent SVG with special handling', async ()=>{
            ...
            const result = await imageProcessor(testImages.transparentSvg, 200, 90, 'jpeg');
            // Check the returned structure
            expect(result).toBeTruthy();
            expect(typeof result).toBe('object');
            expect(result.buffer).toBeInstanceOf(Buffer);
...
describe('Parameter validation', ()=>{
    test('Should throw error with missing src parameter', async ()=>{
        await expect(imageProcessor(null, 200, 80)).rejects.toThrow('Missing or invalid "src", "w", or "h" parameter');
    });
    test('Should throw error with missing width parameter', async ()=>{
        await expect(imageProcessor(testImages.jpg, null, 80)).rejects.toThrow('Missing or invalid "src", "w", or "h" parameter');
    });
    test('Should throw error with NaN width parameter', async ()=>{
        await expect(imageProcessor(testImages.jpg, NaN, 80)).rejects.toThrow('Missing or invalid "src", "w", or "h" parameter');
    });
    test('Should throw error with NaN height parameter', async ()=>{
        await expect(imageProcessor(testImages.jpg, 200, 80, 'webp', NaN)).rejects.toThrow('Missing or invalid "src", "w", or "h" parameter');
    });
});
// Path safety validation tests
describe('Path safety validation', ()=>{
    test('Should throw error with path traversal attempt', async ()=>{
        await expect(imageProcessor('../../../config/config.json', 200, 80)).rejects.toThrow('Invalid characters in image path');
    });
    test('Should throw error with disallowed path', async ()=>{
        await expect(imageProcessor('config/config.json', 200, 80)).rejects.toThrow(/Image source must be from media\/, public\//);
    });
    ...
    test('Should fallback to public directory when asset not in media', async ()=>{
        ...
        try {
            ...
            await fs.unlink(path.join(ROOTPATH, 'media/public-only-asset.png')).catch(()=>{});
    ...
    test('Should default to media path and throw appropriate error when asset not found', async ()=>{
        ...
        await expect(imageProcessor(nonExistentPath, 80, 80, 'webp')).rejects.toThrow(/not found/);
    });
...
describe('Height and upscale options', ()=>{
    ...
    test('Should allow upscaling when allowUpscale=true', async ()=>{
        ...
        const relativeTestPath = path.relative(ROOTPATH, testImagePath);
        const result = await imageProcessor(relativeTestPath, 200, 80, 'png', undefined, true);
...
describe('Cache key generation', ()=>{
    ...
    test('Should generate different cache keys with/without height', async ()=>{
        const relativeTestPath = path.relative(ROOTPATH, testImagePath);
        const result1 = await imageProcessor(relativeTestPath, 200, 80, 'jpeg');
        const result2 = await imageProcessor(relativeTestPath, 200, 80, 'jpeg', 150);
        expect(result1).not.toBe(result2);
    ...
    test('Should generate different cache keys with/without allowUpscale', async ()=>{
        ...
        }).jpeg().toFile(smallTestPath);
        const smallRelativePath = path.relative(ROOTPATH, smallTestPath);
        // Without upscaling (default)
        const result1 = await imageProcessor(smallRelativePath, 200, 80, 'jpeg', undefined, false);
        // With upscaling
        const result2 = await imageProcessor(smallRelativePath, 200, 80, 'jpeg', undefined, true);
        expect(result1).not.toBe(result2);
    ...
    test('Should include appropriate parameters in the cache key', async ()=>{
        const relativeTestPath = path.relative(ROOTPATH, testImagePath);
        const result = await imageProcessor(relativeTestPath, 400, 90, 'webp', 300, true);
...
describe('Error handling', ()=>{
    test('Should handle non-existent local file gracefully', async ()=>{
        await expect(imageProcessor('media/nonexistent.jpg', 200, 80)).rejects.toThrow('Image not found');
    });
    test('Should handle external URL fetch errors gracefully', async ()=>{
        global.fetch = jest.fn().mockRejectedValue(new Error('Network error'));
        await expect(imageProcessor('https://example.com/image.jpg', 200, 80)).rejects.toThrow('Image not found or processing failed');
        delete global.fetch;
    });
    ...
    test('Should reject paths outside allowed directories', async ()=>{
        await expect(imageProcessor('node_modules/package/file.jpg', 200, 80)).rejects.toThrow('Image source must be from');
    });
    test('Should handle corrupt image gracefully', async ()=>{
        ...
        await expect(imageProcessor(relativeCorruptPath, 200, 80)).rejects.toThrow();
        // Clean up
