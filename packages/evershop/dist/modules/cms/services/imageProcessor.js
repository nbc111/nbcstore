...
const hasTransparency = async (imageBuffer)=>{
    try {
        const metadata = await sharp(imageBuffer).metadata();
        return metadata.channels === 4 || metadata.format === 'png' || metadata.format === 'svg';
    } catch  {
        return false;
    }
};
const isSvg = async (imageBuffer)=>{
    try {
        const metadata = await sharp(imageBuffer).metadata();
...
const generateCacheKey = (src, width, quality, format, height, allowUpscale = false)=>{
...
export const imageProcessor = async (src, width, quality, format = 'webp', height, allowUpscale = false)=>{
    if (!src || !width || isNaN(width) || height !== undefined && isNaN(height)) {
    ...
    if (!isExternalUrl) {
        ...
        if (!isAllowedPath) {
            throw new Error('Image source must be from media/, public/, or themes/<themename>/public/ directories at project root');
        }
    ...
    try {
        ...
        let cachedImageKey = generateCacheKey(src, width, quality, format, height, allowUpscale);
        if (isExternalUrl) {
            // Handle external URL
        ...
        cachedImageKey = generateCacheKey(src, width, quality, finalFormat, height, allowUpscale);
        ...
        switch(finalFormat){
            case 'avif':
                optimizedImageBuffer = await sharpInstance.avif({
                    quality,
                    effort: 4,
                ...
                break;
            case 'webp':
                optimizedImageBuffer = await sharpInstance.webp({
                    quality,
                    effort: 4,
                ...
                break;
            case 'jpeg':
                optimizedImageBuffer = await sharpInstance.jpeg({
                    quality,
                    progressive: true,
                ...
                break;
            case 'png':
                optimizedImageBuffer = await sharpInstance.png({
                    quality,
                    compressionLevel: 9,
