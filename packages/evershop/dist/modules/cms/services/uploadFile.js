 ...
 */ export const uploadFile = async (files, destinationPath)=>{
   ...
   */ const fileUploader = getValueSync('fileUploader', localUploader, {
        config: getConfig('system.file_storage')
    }, (value)=>// The value must be an object with an upload method
...
const localUploader = {
    upload: async (files, destinationPath)=>{
        ...
        const results = await Promise.all(files.map((file)=>fs.writeFile(path.join(destination, file.filename), file.buffer).then(()=>({
                    name: file.filename,
                    mimetype: file.mimetype,
                    size: file.size,
                    url: buildUrl('staticAsset', [
                        path.join(destinationPath, file.filename).split('\\').join('/').replace(/^\//, '')
                    ])
                }))));
        return results;
    }
