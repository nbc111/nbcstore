...
export default (async (connection)=>{
    await execute(connection, `CREATE TABLE "cms_page" (
  ...
  CONSTRAINT "CMS_PAGE_UUID" UNIQUE ("uuid")
    ...
    await execute(connection, `CREATE TABLE "cms_page_description" (
  ...
  CONSTRAINT "FK_CMS_PAGE_DESCRIPTION" FOREIGN KEY ("cms_page_description_cms_page_id") REFERENCES "cms_page" ("cms_page_id") ON DELETE CASCADE ON UPDATE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_CMS_PAGE_DESCRIPTION" ON "cms_page_description" ("cms_page_description_cms_page_id")`);
});
