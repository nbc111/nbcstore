...
export default {
    Query: {
        pageInfo: async (root, args, context)=>({
                ...
                title: get(context, 'pageInfo.title', await getSetting('storeName', 'Evershop')),
                description: get(context, 'pageInfo.description', ''),
                keywords: get(context, 'pageInfo.keywords', []),
                canonicalUrl: get(context, 'pageInfo.canonicalUrl', get(context, 'currentUrl')),
                favicon: async ()=>{
    ...
    PageInfo: {
        breadcrumbs: async (root, args, context)=>{
            ...
            const path = context.originalUrl.split('?')[0].replace(/^\/|\/$/g, '').replace(/\./g, '');
            // Check if the path is existed in the url_rewrite table
            const rewriteRule = await select().from('url_rewrite').where('request_path', '=', `/${path}`).load(context.pool);
            if (!rewriteRule) {
                return [
                    {
                ...
                for(let i = 0; i < paths.length; i += 1){
                    ...
                    categoryQuery.leftJoin('category_description').on('category_description.category_description_category_id', '=', 'category.category_id');
        ...
        ogInfo: async (root, args, context)=>{
            ...
            const image = get(context, 'pageInfo.ogInfo.image', logo ? `${baseUrl}/images?src=${logo}&w=1200&q=80&h=675&f=png` : '');
            return getValueSync('ogInfo', {
                title: get(context, 'pageInfo.ogTitle', root.title),
                description: get(context, 'pageInfo.ogInfo.description', root.description),
                image: get(context, 'pageInfo.ogInfo.image', image ? image : root.image),
                ...
                twitterSite: get(context, 'pageInfo.ogInfo.twitterSite', await getSetting('storeName', 'Evershop')),
                twitterCreator: get(context, 'pageInfo.ogInfo.twitterCreator', await getSetting('storeName', 'Evershop')),
