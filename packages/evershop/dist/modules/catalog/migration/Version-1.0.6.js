...
export default (async (connection)=>{
    ...
    await execute(connection, `ALTER TABLE product_image RENAME COLUMN image TO origin_image`);
    // Add  columns 'thumb_image', 'listing_image', 'single_image', 'is_main' to the product_image table if not exist
    await execute(connection, `ALTER TABLE product_image
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION product_image_insert_trigger()
      RETURNS TRIGGER AS $$
      BEGIN
      ...
      END;
      $$ LANGUAGE plpgsql;`);
    await execute(connection, `CREATE TRIGGER "PRODUCT_IMAGE_ADDED"
    ...
    await execute(connection, `UPDATE product_image SET origin_image = CONCAT('/assets', origin_image)`);
    // Add event to the event table for all the product images
    await execute(connection, `INSERT INTO event (name, data)
      SELECT 'product_image_added', row_to_json(product_image)
      FROM product_image`);
    await execute(connection, `INSERT INTO product_image (product_image_product_id, origin_image, is_main)
