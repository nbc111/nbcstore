...
export default (async (connection)=>{
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_product_inventory_updated_event() RETURNS TRIGGER AS $$
    BEGIN
    ...
    await execute(connection, `CREATE TRIGGER "ADD_INVENTORY_UPDATED_EVENT_TRIGGER"
    ...
    const featuredProductsExists = await execute(connection, `SELECT EXISTS (SELECT 1 FROM collection WHERE code = 'homepage');`);
