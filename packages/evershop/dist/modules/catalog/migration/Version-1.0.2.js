...
export default (async (connection)=>{
    ...
    await execute(connection, `ALTER TABLE product ADD COLUMN category_id INT DEFAULT NULL;`);
    // Add a foreign key constraint to the category_id column
    await execute(connection, `ALTER TABLE product ADD CONSTRAINT "PRODUCT_CATEGORY_ID_CONSTRAINT" FOREIGN KEY ("category_id") REFERENCES "category" ("category_id") ON DELETE SET NULL;`);
    // Get 1 category from the product_category table and update product table with according category_id
    await execute(connection, `UPDATE product SET category_id = (SELECT category_id FROM product_category WHERE product_id = product.product_id LIMIT 1);`);
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION build_url_key() RETURNS TRIGGER AS $$
    DECLARE
      url_key TEXT;
    BEGIN
    ...
    await execute(connection, `CREATE TRIGGER "BUILD_CATEGORY_URL_KEY_TRIGGER"
    BEFORE INSERT OR UPDATE ON category_description
    FOR EACH ROW
    EXECUTE PROCEDURE build_url_key();`);
    // Create a trigger to build the url_key from the name if the url_key is not provided
    await execute(connection, `CREATE TRIGGER "BUILD_PRODUCT_URL_KEY_TRIGGER"
    BEFORE INSERT OR UPDATE ON product_description
    FOR EACH ROW
    EXECUTE PROCEDURE build_url_key();`);
    // Create a url_rewrite table to store the url rewrite rules
    await execute(connection, `CREATE TABLE url_rewrite (
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_category_created_event() RETURNS TRIGGER AS $$
    BEGIN
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CATEGORY_CREATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_category_updated_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CATEGORY_UPDATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_category_deleted_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CATEGORY_DELETED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_product_created_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_PRODUCT_CREATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_product_updated_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_PRODUCT_UPDATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_product_deleted_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_PRODUCT_DELETED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE TABLE product_inventory (
    ...
    await execute(connection, `INSERT INTO product_inventory (product_inventory_product_id, qty, manage_stock, stock_availability)
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION delete_sub_categories() RETURNS TRIGGER AS $$
    DECLARE
    ...
    await execute(connection, `CREATE TRIGGER "DELETE_SUB_CATEGORIES_TRIGGER"
    AFTER DELETE ON category
    FOR EACH ROW
    EXECUTE PROCEDURE delete_sub_categories();`);
    // Load all categories and add a category_updated event to the event table
    await execute(connection, `INSERT INTO event (name, data) SELECT 'category_updated', row_to_json(category) FROM category;`);
    // Load all products and add a product_updated event to the event table
    await execute(connection, `INSERT INTO event (name, data) SELECT 'product_updated', row_to_json(product) FROM product;`);
});
