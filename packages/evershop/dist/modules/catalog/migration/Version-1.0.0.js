...
export default (async (connection)=>{
  ...
  CONSTRAINT "FK_CATEGORY_DESCRIPTION" FOREIGN KEY ("category_description_category_id") REFERENCES "category" ("category_id") ON DELETE CASCADE
    ...
    const kids = await insert('category').given({
        status: 1,
        include_in_nav: 1
    }).execute(connection);
    await insert('category_description').given({
    ...
    const women = await insert('category').given({
        status: 1,
        include_in_nav: 1
    }).execute(connection);
    await insert('category_description').given({
    ...
    const men = await insert('category').given({
        status: 1,
        include_in_nav: 1
    }).execute(connection);
    await insert('category_description').given({
