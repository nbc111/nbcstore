...
const buildUrlReWrite = async (data)=>{
    try {
        const categoryUUid = data.uuid;
        const categoryId = data.category_id;
        // Load the category
        const category = await select().from('category').where('category_id', '=', categoryId).load(pool);
        if (!category) {
            return;
        }
        // Load the parent categories
        const parentCategoriesQuery = await execute(pool, `WITH RECURSIVE parent_categories AS (
      ...
      INNER JOIN parent_categories pc ON c.category_id = pc.parent_id
        ...
        for(let i = 0; i < parentCategories.length; i += 1){
            ...
            const urlKey = await select('url_key').from('category_description').where('category_description_category_id', '=', cat.category_id).load(pool);
            path = `/${urlKey.url_key}${path}`;
        }
        // Save the current path
        const currentPath = await select('request_path').from('url_rewrite').where('entity_uuid', '=', categoryUUid).and('entity_type', '=', 'category').load(pool);
        ...
        if (currentPath) {
            await pool.query(`UPDATE url_rewrite SET request_path = REPLACE(request_path, $1, $2) WHERE entity_type IN ('category', 'product') AND entity_uuid != $3`, [
