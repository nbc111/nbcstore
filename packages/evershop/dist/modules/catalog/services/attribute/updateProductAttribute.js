...
import attributeDataSchema from './attributeDataSchema.json' with {
    type: 'json'
};
function validateAttributeDataBeforeInsert(data) {
    const ajv = getAjv();
    attributeDataSchema.required = [];
    const jsonSchema = getValueSync('updateAttributeDataJsonSchema', attributeDataSchema, {});
...
async function updateAttributeOptions(attributeId, attributeType, attributeCode, options, connection) {
    ...
    const ids = options.filter((o)=>o !== undefined).map((o)=>typeof o.option_id === 'string' ? parseInt(o.option_id, 10) : o.option_id);
    const oldOptions = await select().from('attribute_option').where('attribute_id', '=', attributeId).execute(connection, false);
    await Promise.all(oldOptions.map(async (oldOption)=>{
        if (!ids.includes(parseInt(oldOption.attribute_option_id, 10))) {
            await del('attribute_option').where('attribute_option_id', '=', oldOption.attribute_option_id).execute(connection, false);
        }
    }));
    /* Adding new options */ // Looping options array and insert/update options. Use Promise.all to make sure all options are inserted/updated
    await Promise.all(options.map(async (option)=>{
        const exists = await select().from('attribute_option').where('attribute_option_id', '=', option.option_id).load(connection, false);
        if (exists) {
            await update('attribute_option').given({
                option_text: option.option_text,
                attribute_id: attributeId,
                attribute_code: attributeCode
            }).where('attribute_option_id', '=', option.option_id).execute(connection, false);
        } else {
...
async function updateAttributeGroups(attributeId, groups, connection) {
    ...
    currentGroups.forEach((g)=>{
        if (!groups.find((group)=>parseInt(group.toString(), 10) === parseInt(g.group_id.toString(), 10))) {
            shouldDelete.push(g.group_id);
        }
    });
    for(let index = 0; index < groups.length; index += 1){
        const group = await select().from('attribute_group').where('attribute_group_id', '=', groups[index]).load(connection, false);
        if (group) {
    ...
    await del('attribute_group_link').where('group_id', 'IN', shouldDelete).and('attribute_id', '=', attributeId).execute(connection);
}
async function updateAttributeData(uuid, data, connection) {
    ...
    try {
        const updatedAttribute = await update('attribute').given(data).where('uuid', '=', uuid).execute(connection);
 ...
 */ async function updateAttribute(uuid, data, context) {
    ...
    try {
        ...
        await hookable(updateAttributeOptions, {
            ...
            connection,
            attribute
        })(attribute.attribute_id, attribute.type, attribute.attribute_code, attributeData.options || [], connection);
