...
function validateAttributeDataBeforeInsert(data) {
    ...
    const jsonSchema = getValueSync('createAttributeDataJsonSchema', attributeDataSchema, {});
...
async function insertAttributeOptions(attributeId, attributeType, attributeCode, options, connection) {
    ...
    /* Adding new options */ await Promise.all(options.map(async (option)=>{
 ...
 */ async function createAttribute(data, context) {
    ...
    try {
        ...
        await hookable(insertAttributeOptions, {
            ...
            attribute,
            connection
        })(attribute.insertId, attribute.type, attribute.attribute_code, attributeData.options || [], connection);
