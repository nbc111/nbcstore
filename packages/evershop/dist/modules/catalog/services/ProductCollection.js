   ...
   */ async init(filters = [], isAdmin = false) {
        ...
        if (!isAdmin) {
            ...
            if (getConfig('catalog.showOutOfStockProduct', false) === false) {
                this.baseQuery.andWhere('product_inventory.manage_stock', '=', false).addNode(node('OR').addLeaf('AND', 'product_inventory.qty', '>', 0).addLeaf('AND', 'product_inventory.stock_availability', '=', true));
            }
        }
        const currentFilters = [];
        // Attribute filter
        const filterableAttributes = await select().from('attribute').where('type', '=', 'select').and('is_filterable', '=', 1).execute(pool);
        // Apply the filters
        const productCollectionFilters = await getValue('productCollectionFilters', [], {
            isAdmin,
            filterableAttributes
        });
        productCollectionFilters.forEach((filter)=>{
            const check = filters && filters.find((f)=>f.key === filter.key && filter.operation.includes(f.operation));
            if (filter.key === '*' || check) {
        ...
        if (!isAdmin) {
            ...
            const visibleGroups = (await select('variant_group_id').from('variant_group').where('visibility', '=', 't').execute(pool)).map((v)=>v.variant_group_id);
            if (visibleGroups) {
                // Get all invisible variants from current query
                copy.select('bool_or(product.visibility)', 'sumv').select('max(product.product_id)', 'product_id').andWhere('product.variant_group_id', 'IN', visibleGroups);
                ...
                if (invisibleIds.length > 0) {
                    const n = node('AND');
                    n.addLeaf('AND', 'product.product_id', 'IN', invisibleIds).addNode(node('OR').addLeaf('OR', 'product.visibility', '=', 't'));
            ...
            onePerVariantGroupQuery.select(sql('DISTINCT ON (COALESCE(product.variant_group_id, random())) product.product_id', 'product_id'));
            onePerVariantGroupQuery.removeOrderBy();
            const onePerGroup = await onePerVariantGroupQuery.execute(pool);
            this.baseQuery.andWhere('product.product_id', 'IN', onePerGroup.map((v)=>v.product_id));
        }
