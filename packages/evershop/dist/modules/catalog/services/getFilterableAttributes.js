...
export const getFilterableAttributes = async (categoryId)=>{
    ...
    const query = select('attribute.attribute_name', 'attribute_name').select('attribute.type', 'type').select('attribute.is_filterable', 'is_filterable').select('product_attribute_value_index.attribute_id', 'attribute_id').select('attribute.attribute_code', 'attribute_code').select('product_attribute_value_index.option_id', 'option_id').select('product_attribute_value_index.option_text', 'option_text').from('attribute');
    query.innerJoin('product_attribute_value_index').on('attribute.attribute_id', '=', 'product_attribute_value_index.attribute_id');
    query.where('product_attribute_value_index.product_id', 'IN', allIds).and('type', '=', 'select').and('is_filterable', '=', 1);
    ...
    for(let i = 0; i < attributeData.length; i++){
        ...
        if (index === -1) {
            ...
            const idx = attributes[index].options.findIndex((o)=>parseInt(o.optionId, 10) === parseInt(attributeData[i].option_id, 10));
            if (idx === -1) {
