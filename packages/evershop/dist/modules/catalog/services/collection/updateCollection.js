...
async function updateCollectionData(uuid, data, connection) {
    const collection = await select().from('collection').where('uuid', '=', uuid).load(connection);
    if (!collection) {
        throw new Error('Requested collection not found');
    }
    try {
        const newCollection = await update('collection').given(data).where('uuid', '=', uuid).execute(connection);
 ...
 */ async function updateCollection(uuid, data, context) {
    ...
    try {
        ...
        const collection = await hookable(updateCollectionData, hookContext)(uuid, collectionData, connection);
 ...
 */ export default (async (uuid, data, context)=>{
    ...
    const collection = await hookable(updateCollection, context)(uuid, data, context);
    return collection;
});
export function hookBeforeUpdateCollectionData(callback, priority = 10) {
    hookBefore('updateCollectionData', callback, priority);
}
export function hookAfterUpdateCollectionData(callback, priority = 10) {
    hookAfter('updateCollectionData', callback, priority);
}
export function hookBeforeUpdateCollection(callback, priority = 10) {
    hookBefore('updateCollection', callback, priority);
}
export function hookAfterUpdateCollection(callback, priority = 10) {
