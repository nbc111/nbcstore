...
export class AttributeGroupCollection {
    ...
    async init(filters = []) {
        ...
        const defaultFilters = [
            {
                ...
                callback: (query, operation, value, currentFilters)=>{
                    query.andWhere('attribute_group.group_name', OPERATION_MAP[operation], value);
                    currentFilters.push({
                        key: 'name',
                        operation,
                        value
        ...
        // Apply the filters
        const attributeGroupCollectionFilters = await getValue('attributeGroupCollectionFilters', defaultFilters);
        attributeGroupCollectionFilters.forEach((filter)=>{
            const check = filters.find((f)=>f.key === filter.key && filter.operation.includes(f.operation));
            if (filter.key === '*' || check) {
