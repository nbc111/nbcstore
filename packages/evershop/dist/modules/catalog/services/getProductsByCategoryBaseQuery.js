...
export const getProductsByCategoryBaseQuery = async (categoryId, fromSubCategories = false)=>{
    const query = getProductsBaseQuery();
    if (!fromSubCategories) {
        query.where('product.category_id', '=', categoryId);
    } else {
        // Get all the sub categories recursively
        const subCategoriesQuery = await execute(pool, `WITH RECURSIVE sub_categories AS (
