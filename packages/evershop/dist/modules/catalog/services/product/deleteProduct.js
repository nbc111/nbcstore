 ...
 */ async function deleteProduct(uuid, context) {
    ...
    try {
        ...
        await hookable(deleteProductData, {
            ...context,
            connection,
            product
        })(uuid, connection);
 ...
 */ export default (async (uuid, context)=>{
    ...
    try {
        ...
        const product = await hookable(deleteProduct, hookContext)(uuid, connection);
        await commit(connection);
        return product;
    } catch (e) {
        await rollback(connection);
...
export function hookBeforeDeleteProductData(callback, priority = 10) {
    hookBefore('deleteProductData', callback, priority);
}
export function hookAfterDeleteProductData(callback, priority = 10) {
    hookAfter('deleteProductData', callback, priority);
}
export function hookBeforeDeleteProduct(callback, priority = 10) {
    hookBefore('deleteProduct', callback, priority);
}
export function hookAfterDeleteProduct(callback, priority = 10) {
