...
async function insertProductInventory(inventoryData, productId, connection) {
    ...
    await insert('product_inventory').given(inventoryData).prime('product_inventory_product_id', productId).execute(connection);
}
...
async function insertProductImages(images, productId, connection) {
    ...
    await Promise.all(images.map((f, index)=>(async ()=>{
            ...
            await insert('product_image').given({
                ...
                is_main: index === 0
            }).prime('product_image_product_id', productId).execute(connection);
        })()));
}
async function insertProductData(data, connection) {
    ...
    const description = await insert('product_description').given(productData).prime('product_description_product_id', product.product_id).execute(connection);
    return {
        ...description,
        ...product
    };
 ...
 */ async function createProduct(data, context) {
    ...
    try {
        ...
        await hookable(insertProductInventory, {
            ...context,
            connection,
            product
        })(productData, product.insertId, connection);
        ...
        await hookable(insertProductImages, {
            ...context,
            connection,
            product
        })(productData.images || [], product.insertId, connection);
...
export function hookBeforeInsertProductData(callback, priority = 10) {
    hookBefore('insertProductData', callback, priority);
}
export function hookAfterInsertProductData(callback, priority = 10) {
    hookAfter('insertProductData', callback, priority);
}
export function hookBeforeInsertProductInventory(callback, priority = 10) {
    hookBefore('insertProductInventory', callback, priority);
}
export function hookAfterInsertProductInventory(callback, priority = 10) {
    hookAfter('insertProductInventory', callback, priority);
}
export function hookBeforeInsertProductAttributes(callback, priority = 10) {
    hookBefore('insertProductAttributes', callback, priority);
}
export function hookAfterInsertProductAttributes(callback, priority = 10) {
    hookAfter('insertProductAttributes', callback, priority);
}
export function hookBeforeInsertProductImages(callback, priority = 10) {
    hookBefore('insertProductImages', callback, priority);
}
export function hookAfterInsertProductImages(callback, priority = 10) {
    hookAfter('insertProductImages', callback, priority);
}
export function hookBeforeCreateProduct(callback, priority = 10) {
    hookBefore('createProduct', callback, priority);
}
export function hookAfterCreateProduct(callback, priority = 10) {
