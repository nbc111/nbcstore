...
export default (async (request, response, next)=>{
    ...
    try {
        ...
        const attributes = await select().from('attribute').where('attribute_code', 'in', attribute_codes).and('type', '=', 'select').execute(pool);
        ...
        const attributeGroupLinks = await select().from('attribute_group_link').where('group_id', '=', attribute_group_id).and('attribute_id', 'in', attributes.map((a)=>a.attribute_id)).execute(pool);
        ...
        const group = await select().from('variant_group').where('variant_group_id', '=', result.insertId).load(pool);
        const promises = attributes.map(async (attribute)=>{
            const { attribute_id } = attribute;
            const options = await select().from('attribute_option').where('attribute_id', '=', attribute_id).execute(pool);
            return {
                ...attribute,
                options
            };
