        ...
        currentCategory: async (_, args, { currentUrl, currentRoute, pool })=>{
            ...
            return result ? {
                ...
                products: async (_, { filters = [] })=>{
                    const query = await getProductsByCategoryBaseQuery(result.category_id, true);
    ...
    Category: {
        products: async (category, { filters = [] }, { user })=>{
            ...
            if (typeof category.products === 'function') {
                return await category.products(category, {
                    filters
                });
            }
            const query = await getProductsByCategoryBaseQuery(category.categoryId, !user);
            const root = new ProductCollection(query);
            await root.init(filters, !!user);
            return root;
        },
        ...
        priceRange: async (category, _, { pool })=>{
            ...
            query.select('MIN(product.price)', 'min').select('MAX(product.price)', 'max');
            const result = await query.load(pool);
            return {
                min: result.min || 0,
        ...
        url: async (category, _, { pool })=>{
            ...
            const urlRewrite = await select().from('url_rewrite').where('entity_uuid', '=', category.uuid).and('entity_type', '=', 'category').load(pool);
            if (!urlRewrite) {
        ...
        children: async (category, _, { pool })=>{
            const query = select().from('category');
            query.leftJoin('category_description', 'des').on('des.category_description_category_id', '=', 'category.category_id');
            query.where('category.parent_id', '=', category.categoryId);
            const results = await query.execute(pool);
            return results.map((row)=>camelCase(row));
        },
        path: async (category, _, { pool })=>{
            const query = await execute(pool, `WITH RECURSIVE category_path AS (
