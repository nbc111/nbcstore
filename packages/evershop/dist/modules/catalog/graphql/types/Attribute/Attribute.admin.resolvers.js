...
export default {
    ...
    AttributeGroup: {
        ...
        updateApi: (group)=>buildUrl('updateAttributeGroup', {
                id: group.uuid
            })
    },
    Attribute: {
        groups: async (attribute, { filters = [] })=>{
            const query = getAttributeGroupsBaseQuery();
            query.innerJoin('attribute_group_link').on('attribute_group.attribute_group_id', '=', 'attribute_group_link.group_id');
            query.where('attribute_group_link.attribute_id', '=', attribute.attributeId);
            const root = new AttributeGroupCollection(query);
            await root.init(filters);
            return root;
        },
        editUrl: ({ uuid })=>buildUrl('attributeEdit', {
                id: uuid
            }),
        updateApi: (attribute)=>buildUrl('updateAttribute', {
                id: attribute.uuid
            }),
        deleteApi: (attribute)=>buildUrl('deleteAttribute', {
                id: attribute.uuid
            })
    }
};
