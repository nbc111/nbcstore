...
export default {
    Product: {
        url: async (product, _, { pool })=>{
            ...
            const urlRewrite = await select().from('url_rewrite').where('entity_uuid', '=', product.uuid).and('entity_type', '=', 'product').load(pool);
            if (!urlRewrite) {
    ...
    Query: {
        ...
        currentProduct: async (_, args, { currentRoute, currentProductId, pool })=>{
