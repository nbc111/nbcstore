...
export default (async (request, response, next)=>{
    try {
        ...
        if (stripePaymentMode === 'capture' && paymentIntent.status === 'succeeded' || stripePaymentMode === 'authorizeOnly' && paymentIntent.status === 'requires_capture') {
            // Redirect to the order success page
            response.redirect(buildUrl('checkoutSuccess', {
                orderId: order_id
            }));
            return;
        } else {
