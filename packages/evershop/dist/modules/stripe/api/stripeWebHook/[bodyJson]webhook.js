import { startTransaction, commit, rollback, select, insertOnUpdate } from '@evershop/postgres-query-builder';
...
export default (async (request, response, next)=>{
    ...
    try {
        ...
        const transaction = await select().from('payment_transaction').where('transaction_id', '=', paymentIntent.id).load(connection);
        // Load the order
        const order = await select().from('order').where('uuid', '=', order_id).load(connection);
        if (!order) {
            ...
            case 'payment_intent.succeeded':
                {
                    ...
                    await insertOnUpdate('payment_transaction', [
                        ...
                        payment_action: paymentIntent.capture_method === 'manual' ? 'Manual' : 'Automatic'
                    }).execute(connection);
                    if (!transaction) {
                        await updatePaymentStatus(order.order_id, 'stripe_captured', connection);
                        // Add an activity log
                        await addOrderActivityLog(order.order_id, `Payment captured by using Stripe. Transaction ID: ${paymentIntent.id}`, false, connection);
            ...
            case 'payment_intent.amount_capturable_updated':
                {
                    ...
                    await insertOnUpdate('payment_transaction', [
                        ...
                        transaction_id: paymentIntent.id,
                        transaction_type: 'online',
                        payment_action: paymentIntent.capture_method === 'manual' ? 'authorize' : 'capture'
                    }).execute(connection);
                    if (!transaction) {
                        await updatePaymentStatus(order.order_id, 'stripe_authorized', connection);
                        // Add an activity log
                        await addOrderActivityLog(order.order_id, `Payment authorized by using Stripe. Transaction ID: ${paymentIntent.id}`, false, connection);
                        // Emit event to add order placed event
                        await emit('order_placed', {
                            ...order
                        });
