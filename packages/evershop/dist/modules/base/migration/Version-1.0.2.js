...
export default (async (connection)=>{
    ...
    await execute(connection, `ALTER TABLE "event"
      ADD COLUMN "status" varchar NOT NULL DEFAULT 'pending',
      ADD COLUMN "started_at" TIMESTAMP WITH TIME ZONE,
      ADD COLUMN "completed_at" TIMESTAMP WITH TIME ZONE`);
    // Index for efficient polling: only scan pending rows
    await execute(connection, `CREATE INDEX "EVENT_STATUS_IDX" ON "event" ("status", "event_id" ASC)`);
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION notify_new_event() RETURNS trigger AS $$
BEGIN
  PERFORM pg_notify('new_event', NEW.name);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql`);
    await execute(connection, `CREATE TRIGGER event_inserted
