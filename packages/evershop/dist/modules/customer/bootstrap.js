...
export default (()=>{
        ...
        fields.push({
            ...
            resolvers: [
                async function resolver() {
                    ...
                    return triggeredField === 'customer_id' ? requestedValue : this.getData('customer_id');
                }
            ]
        });
        fields.push({
            ...
            resolvers: [
                async function resolver() {
                    const triggeredField = this.getTriggeredField();
                    const requestedValue = this.getRequestedValue();
                    return triggeredField === 'customer_group_id' ? requestedValue : this.getData('customer_group_id');
                }
            ]
        });
        fields.push({
            ...
            resolvers: [
                async function resolver() {
                    const triggeredField = this.getTriggeredField();
                    const requestedValue = this.getRequestedValue();
                    return triggeredField === 'customer_email' ? requestedValue : this.getData('customer_email');
                }
            ]
        });
        fields.push({
            ...
            resolvers: [
                async function resolver() {
                    const triggeredField = this.getTriggeredField();
                    const requestedValue = this.getRequestedValue();
                    return triggeredField === 'customer_full_name' ? requestedValue : this.getData('customer_full_name');
                }
            ]
        });
        return fields;
   ...
   */ request.loginCustomerWithEmail = async function login(email, password, callback) {
    ...
    addProcessor('customerCollectionFilters', (filters)=>[
            ...filters,
    ...
    addProcessor('customerGroupCollectionFilters', (filters)=>[
            ...filters,
            ...defaultPaginationFilters
        ], 2);
});
