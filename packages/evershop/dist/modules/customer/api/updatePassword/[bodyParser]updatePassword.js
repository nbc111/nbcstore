...
export default (async (request, response, next)=>{
    ...
    try {
        ...
        const resetTokenLifetime = getConfig('resetPasswordTokenLifetime', 48 * 60 * 60 * 1000);
        // Convert Date object to mysql timestamp
        dayjs.extend(utc);
        dayjs.extend(timezone);
        const timezoneConfig = getConfig('shop.timezone', 'UTC');
        const now = dayjs.tz(new Date(Date.now() - resetTokenLifetime), timezoneConfig).format('YYYY-MM-DD HH:mm:ss');
        const existingToken = await select().from('reset_password_token').where('token', '=', hash).and('created_at', '>=', now).load(pool);
        if (!existingToken) {
        ...
        // Delete the token
        await del('reset_password_token').where('reset_password_token_id', '=', existingToken.reset_password_token_id).execute(pool);
        response.status(OK);
        response.$body = {};
        next();
    } catch (e) {
