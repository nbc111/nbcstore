...
export default (async (request, response, next)=>{
    ...
    try {
        const customer = await select().from('customer').where('uuid', '=', request.params.id).load(connection, false);
        if (!customer) {
        ...
        }).where('uuid', '=', request.params.id).execute(connection, false);
        // Load updated customer
        const updatedCustomer = await select().from('customer').where('uuid', '=', request.params.id).load(connection);
        response.status(OK);
        response.$body = {
            data: {
                ...updatedCustomer,
