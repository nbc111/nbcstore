...
`;
export default async function sendCustomerWelcomeEmail(data) {
    try {
        ...
        if (config?.templatePath) {
            ...
            try {
                ...
                debug(`Customer welcome email template file not found at path: ${filePath}. Using default template.`);
                template = TEMPLATE;
            }
        } else {
            template = TEMPLATE;
        ...
        const args = await getValue('customerWelcomeEmailArguments', {
            to: email,
            subject,
            template,
            data: dynamicData
