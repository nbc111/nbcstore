...
export default (async (connection)=>{
    await execute(connection, `CREATE TABLE "customer_group" (
  ...
  "updated_at" TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
    ...
    await execute(connection, "INSERT INTO customer_group ( group_name ) VALUES ('Default')");
  ...
  CONSTRAINT "FK_CUSTOMER_GROUP" FOREIGN KEY ("group_id") REFERENCES "customer_group" ("customer_group_id") ON DELETE SET NULL
    ...
    await execute(connection, `CREATE INDEX "FK_CUSTOMER_GROUP" ON "customer" ("group_id")`);
    await execute(connection, `CREATE TABLE "customer_address" (
  ...
  CONSTRAINT "FK_CUSTOMER_ADDRESS" FOREIGN KEY ("customer_id") REFERENCES "customer" ("customer_id") ON DELETE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_CUSTOMER_ADDRESS" ON "customer_address" ("customer_id")`);
    // Prevent deleting a default customer group
    await execute(connection, `CREATE OR REPLACE FUNCTION prevent_delete_default_customer_group()
        RETURNS TRIGGER
        LANGUAGE PLPGSQL
        AS
      $$
      ...
      END;
      $$`);
    await execute(connection, `CREATE TRIGGER "PREVENT_DELETING_THE_DEFAULT_CUSTOMER_GROUP"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION set_default_customer_group()
        RETURNS TRIGGER
        LANGUAGE PLPGSQL
        AS
      $$
      ...
      END;
      $$`);
    await execute(connection, `CREATE TRIGGER "SET_DEFAULT_CUSTOMER_GROUP"
