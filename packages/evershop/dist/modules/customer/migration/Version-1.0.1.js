...
export default (async (connection)=>{
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_customer_created_event() RETURNS TRIGGER AS $$
    BEGIN
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CUSTOMER_CREATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_customer_updated_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CUSTOMER_UPDATED_EVENT_TRIGGER"
    ...
    await execute(connection, `CREATE OR REPLACE FUNCTION add_customer_deleted_event() RETURNS TRIGGER AS $$
    BEGIN
      INSERT INTO event (name, data)
    ...
    await execute(connection, `CREATE TRIGGER "ADD_CUSTOMER_DELETED_EVENT_TRIGGER"
