...
async function updateCustomerPassword(customerId, hash, connection) {
    await update('customer').given({
        password: hash
    }).where('customer_id', '=', customerId).execute(connection);
}
/**
 ...
 */ async function updatePassword(customerId, newPassword, context) {
    ...
    try {
        ...
        const customer = await query.where('customer_id', '=', customerId).load(connection);
        if (!customer) {
...
export function hookBeforeUpdateCustomerPassword(callback, priority = 10) {
    hookBefore('updateCustomerPassword', callback, priority);
}
export function hookAfterUpdateCustomerPassword(callback, priority = 10) {
    hookAfter('updateCustomerPassword', callback, priority);
}
export function hookBeforeUpdatePassword(callback, priority = 10) {
    hookBefore('updatePassword', callback, priority);
}
export function hookAfterUpdatePassword(callback, priority = 10) {
