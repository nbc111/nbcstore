...
async function insertCustomerAddressData(data, connection) {
    const address = await insert('customer_address').given(data).execute(connection);
    if (address.is_default) {
        await update('customer_address').given({
            is_default: 0
        }).where('customer_id', '=', address.customer_id).and('uuid', '<>', address.uuid).execute(connection);
    }
    return address;
}
/**
 ...
 */ async function createCustomerAddress(customerUUID, address, context) {
    ...
    try {
        const customerAddressData = await getValue('customerAddressDataBeforeCreate', address, context);
        ...
        const customer = await select().from('customer').where('uuid', '=', customerUUID).load(pool);
        if (!customer) {
 ...
 */ export default (async (customerUUID, addressData, context)=>{
    ...
    const address = await hookable(createCustomerAddress, context)(customerUUID, addressData, context);
    return address;
});
export function hookBeforeInsertCustomerAddressData(callback, priority = 10) {
    hookBefore('insertCustomerAddressData', callback, priority);
}
export function hookAfterInsertCustomerAddressData(callback, priority = 10) {
    hookAfter('insertCustomerAddressData', callback, priority);
}
export function hookBeforeCreateCustomerAddress(callback, priority = 10) {
    hookBefore('createCustomerAddress', callback, priority);
}
export function hookAfterCreateCustomerAddress(callback, priority = 10) {
