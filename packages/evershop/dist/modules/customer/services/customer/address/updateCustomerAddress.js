...
async function updateCustomerAddressData(uuid, data, connection) {
    ...
    try {
        const newAddress = await update('customer_address').given(data).where('uuid', '=', uuid).execute(connection);
        if (newAddress.is_default) {
            await update('customer_address').given({
                is_default: 0
            }).where('customer_id', '=', newAddress.customer_id).and('uuid', '<>', newAddress.uuid).execute(connection);
        }
 ...
 */ async function updateCustomerAddress(uuid, data, context) {
    ...
    try {
        ...
        const query = select().from('customer_address');
        const currentAddress = await query.where('uuid', '=', uuid).load(connection);
        if (!currentAddress) {
 ...
 */ export default (async (uuid, data, context)=>{
    ...
    const address = await hookable(updateCustomerAddress, context)(uuid, data, context);
    return address;
});
export function hookBeforeUpdateCustomerAddressData(callback, priority = 10) {
    hookBefore('updateCustomerAddressData', callback, priority);
}
export function hookAfterUpdateCustomerAddressData(callback, priority = 10) {
    hookAfter('updateCustomerAddressData', callback, priority);
}
export function hookBeforeUpdateCustomerAddress(callback, priority = 10) {
    hookBefore('updateCustomerAddress', callback, priority);
}
export function hookAfterUpdateCustomerAddress(callback, priority = 10) {
