...
import { hashPassword, verifyPassword } from '../../../../lib/util/passwordHelper.js';
import { getValue, getValueSync } from '../../../../lib/util/registry.js';
import { getAjv } from '../../../base/services/getAjv.js';
import customerDataSchema from './customerDataSchema.json' with {
    type: 'json'
};
function validateCustomerDataBeforeInsert(data) {
    ...
    const jsonSchema = getValueSync('createCustomerDataJsonSchema', customerDataSchema, {});
 ...
 */ async function createCustomer(data, context = {}) {
    ...
    try {
        ...
        const existingCustomer = await select().from('customer').where('email', '=', email).load(pool);
        if (existingCustomer) {
            throw new Error('Email is already used');
        }
        customerData.status = customerData.status !== undefined ? customerData.status : 1;
        customerData.password = hashedPassword;
        customerData.group_id = customerData.group_id !== undefined ? customerData.group_id : 1;
...
export function hookBeforeInsertCustomerData(callback, priority = 10) {
    hookBefore('insertCustomerData', callback, priority);
}
export function hookAfterInsertCustomerData(callback, priority = 10) {
    hookAfter('insertCustomerData', callback, priority);
}
export function hookBeforeCreateCustomer(callback, priority = 10) {
    hookBefore('createCustomer', callback, priority);
}
export function hookAfterCreateCustomer(callback, priority = 10) {
