...
function validateCustomerDataBeforeInsert(data) {
    ...
    const jsonSchema = getValueSync('updateCustomerDataJsonSchema', customerDataSchema, {});
...
async function updateCustomerData(uuid, data, connection) {
    ...
    try {
        const newCustomer = await update('customer').given(data).where('uuid', '=', uuid).execute(connection);
...
export function hookBeforeUpdateCustomerData(callback, priority = 10) {
    hookBefore('updateCustomerData', callback, priority);
}
export function hookAfterUpdateCustomerData(callback, priority = 10) {
    hookAfter('updateCustomerData', callback, priority);
}
export function hookBeforeUpdateCustomer(callback, priority = 10) {
    hookBefore('updateCustomer', callback, priority);
}
export function hookAfterUpdateCustomer(callback, priority = 10) {
