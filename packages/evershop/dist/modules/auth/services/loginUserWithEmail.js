...
const _loginUserWithEmail = async function loginUserWithEmail(email, password) {
    // Escape the email to prevent SQL injection
    const userEmail = email.replace(/%/g, '\\%');
    const user = await select().from('admin_user').where('email', 'ILIKE', userEmail).and('status', '=', 1).load(pool);
...
export function hookBeforeLoginUserWithEmail(callback, priority = 10) {
    hookBefore('loginUserWithEmail', callback, priority);
}
export function hookAfterLoginUserWithEmail(callback, priority = 10) {
