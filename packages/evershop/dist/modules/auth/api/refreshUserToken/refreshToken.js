...
import { INVALID_PAYLOAD, UNAUTHORIZED } from '../../../../lib/util/httpStatus.js';
import { generateToken, TOKEN_TYPES, verifyRefreshToken } from '../../../../lib/util/jwt.js';
...
export default (async (request, response, next)=>{
    ...
    try {
        ...
        const adminUser = await select().from('admin_user').where('admin_user_id', '=', decoded.user.admin_user_id).and('status', '=', 1).load(pool);
        if (!adminUser) {
