    ...
    const defaultFilters = [
        {
            ...
            callback: (query, operation, value, currentFilters)=>{
                query.andWhere('order.customer_email', 'ILIKE', `%${value}%`).or('order.order_number', 'ILIKE', `%${value}%`).or('order.customer_full_name', 'ILIKE', `%${value}%`);
                currentFilters.push({
                    key: 'keyword',
                    operation,
                    value
        ...
        },
        {
            ...
            callback: (query, operation, value, currentFilters)=>{
                if (operation === 'eq') {
                    query.andWhere('order.customer_email', OPERATION_MAP[operation], value);
                } else {
                    query.andWhere('order.customer_email', OPERATION_MAP[operation], `%${value}%`);
                }
                currentFilters.push({
        ...
        },
        {
            ...
            callback: (query, operation, value, currentFilters)=>{
                query.andWhere('order.shipment_status', OPERATION_MAP[operation], value);
                currentFilters.push({
                    key: 'shipment_status',
                    operation,
                    value
