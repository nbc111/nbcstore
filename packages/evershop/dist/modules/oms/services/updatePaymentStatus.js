...
async function changePaymentStatus(orderId, status, connection) {
    const order = await update('order').given({
...
export const updatePaymentStatus = async (orderId, status, conn)=>{
    ...
    try {
        ...
        hookable(validatePaymentStatusBeforeUpdate, {
            orderId
        })(status);
        await hookable(changePaymentStatus, {
            orderId,
            status
...
export function hookBeforeValidatePaymentStatusBeforeUpdate(callback, priority = 10) {
    hookBefore('validatePaymentStatusBeforeUpdate', callback, priority);
}
export function hookAfterValidatePaymentStatusBeforeUpdate(callback, priority = 10) {
    hookAfter('validatePaymentStatusBeforeUpdate', callback, priority);
}
export function hookBeforeChangePaymentStatus(callback, priority = 10) {
    hookBefore('changePaymentStatus', callback, priority);
}
export function hookAfterChangePaymentStatus(callback, priority = 10) {
