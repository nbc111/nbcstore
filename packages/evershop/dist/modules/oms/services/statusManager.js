 ...
 */ function getOrderStatusList() {
    const statuses = getConfig('oms.order.status', {});
    return statuses;
}
/**
 ...
 */ function getShipmentStatusList() {
    const statuses = getConfig('oms.order.shipmentStatus', {});
    return statuses;
}
/**
 ...
 */ function registerPaymentStatus(id, detail, psoMapping) {
    // Make sure ID is not empty and does not contain spaces
    if (!id || /\s/.test(id)) {
        throw new Error('Payment status ID must be a non-empty string without spaces.');
    }
    const statusConfig = {
        order: {
            paymentStatus: {
 ...
 */ function registerShipmentStatus(id, detail, psoMapping) {
    // Make sure ID is not empty and does not contain spaces
    if (!id || /\s/.test(id)) {
        throw new Error('Shipment status ID must be a non-empty string without spaces.');
    }
    const statusConfig = {
        order: {
            shipmentStatus: {
 ...
 */ function registerOrderStatus(id, detail) {
    // Make sure ID is not empty and does not contain spaces
    if (!id || /\s/.test(id)) {
        throw new Error('Order status ID must be a non-empty string without spaces.');
    }
    const statusConfig = {
        order: {
            status: {
 ...
 */ function registerPSOStatusMapping(paymentStatus, shipmentStatus, orderStatus) {
    addProcessor('psoMapping', async (mapping)=>{
        mapping[`${paymentStatus}:${shipmentStatus}`] = orderStatus;
        return mapping;
    });
}
export { getOrderStatusList, getShipmentStatusList, getPaymentStatusList, registerPaymentStatus, registerShipmentStatus, registerOrderStatus, registerPSOStatusMapping };
