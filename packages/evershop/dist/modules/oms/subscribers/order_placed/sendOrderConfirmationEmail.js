...
export default async function sendOrderConfirmationEmail(data) {
    try {
        ...
        if (!data.no_shipping_required) {
            shippingAddress.country_name = countries.find((c)=>c.code === shippingAddress.country)?.name || '';
            shippingAddress.province_name = provinces.find((p)=>p.code === shippingAddress.province)?.name || '';
        }
        const billingAddress = await select().from('order_address').where('order_address_id', '=', order.billing_address_id).load(pool);
        billingAddress.country_name = countries.find((c)=>c.code === billingAddress.country)?.name || '';
        billingAddress.province_name = provinces.find((p)=>p.code === billingAddress.province)?.name || '';
        let template;
        if (config?.templatePath) {
            ...
            try {
                ...
                debug(`Order confirmation email template file not found at path: ${filePath}. Using default template.`);
                template = TEMPLATE;
            }
        } else {
            template = TEMPLATE;
        ...
        if (data.customer_email) {
            const args = await getValue('orderConfirmationEmailArguments', {
