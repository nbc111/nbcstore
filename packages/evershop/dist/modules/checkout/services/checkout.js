...
const _checkout = async function checkout(cartId, data, context = {}) {
    ...
    if (data.shippingAddress) {
        const shippingAddress = await addShippingAddress(cart.getData('uuid'), data.shippingAddress, context);
        await cart.setData('shipping_address_id', shippingAddress.cart_address_id);
    }
    // Add Billing Address
    if (data.billingAddress) {
        const billingAddress = await addBillingAddress(cart.getData('uuid'), data.billingAddress, context);
 ...
 */ export async function checkout(cartId, data, context = {}) {
...
export function hookBeforeCheckout(callback, priority = 10) {
    hookBefore('checkout', callback, priority);
}
export function hookAfterCheckout(callback, priority = 10) {
