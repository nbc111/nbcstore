            ...
            resolvers: [
                async function resolver() {
                    ...
                    if (catalogPriceInclTax) {
                        const taxAmount = calculateTaxAmount(this.getData('tax_percent'), product.price, 1, true);
        ...
        },
        {
            ...
            resolvers: [
                async function resolver() {
                    const catalogPriceInclTax = getConfig('pricing.tax.price_including_tax', false);
                    if (catalogPriceInclTax) {
                        return calculateTaxAmount(this.getData('tax_percent'), this.getData('product_price_incl_tax'), this.getData('qty'), true);
                    } else {
                        return calculateTaxAmount(this.getData('tax_percent'), this.getData('product_price'), this.getData('qty'));
                    }
                }
            ],
            dependencies: [
        ...
        },
        {
            ...
            resolvers: [
                async function resolver() {
                    ...
                    const finalPricePerUnit = priceIncludingTax ? this.getData('final_price_incl_tax') - discountAmountPerUnit : this.getData('final_price') - discountAmountPerUnit;
                    return calculateTaxAmount(this.getData('tax_percent'), finalPricePerUnit, this.getData('qty'), priceIncludingTax);
                }
            ],
            dependencies: [
                'discount_amount',
            ...
            resolvers: [
                async function resolver() {
                    ...
                    if (catalogPriceInclTax) {
                        ...
                        const taxAmount = calculateTaxAmount(this.getData('tax_percent'), this.getData('product_price'), 1);
