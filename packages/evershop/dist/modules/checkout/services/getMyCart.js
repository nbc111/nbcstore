 ...
 */ export const getMyCart = async (sid, customerId)=>{
    // Try to get the cart by the session id first
    const cartBySid = await select().from('cart').where('status', '=', 1).andWhere('sid', '=', sid).load(pool);
    if (cartBySid) {
        ...
        if (customerId) {
            ...
            const abandonedCart = await select().from('cart').where('customer_id', '=', customerId).andWhere('status', '=', 1).load(pool);
            if (abandonedCart) {
                // Update the cart with the session id
                await update('cart').given({
                    sid: sid
                }).where('uuid', '=', abandonedCart.uuid).execute(pool);
