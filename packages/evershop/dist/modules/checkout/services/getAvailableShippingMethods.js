...
export const getAvailableShippingMethods = async (cartId, country, province, postcode)=>{
    const cart = await select().from('cart').where('uuid', '=', cartId).and('status', '=', 't').load(pool);
    if (!cart) {
        throw new Error('Cart not found');
    }
    const shippingAddress = await select().from('cart_address').where('cart_address_id', '=', cart.shipping_address_id).load(pool);
    if (!country && !shippingAddress?.country) {
        return [];
    }
    const zoneQuery = select().from('shipping_zone');
    zoneQuery.leftJoin('shipping_zone_province').on('shipping_zone_province.zone_id', '=', 'shipping_zone.shipping_zone_id');
    if (province || shippingAddress?.province) {
        zoneQuery.where('shipping_zone_province.province', '=', province || shippingAddress?.province).or('shipping_zone_province.province', 'IS NULL', null);
    } else {
        zoneQuery.where('shipping_zone_province.province', 'IS NULL', null);
    }
    zoneQuery.andWhere('shipping_zone.country', '=', country || shippingAddress?.country);
    const zone = await zoneQuery.load(pool);
    if (!zone) {
        return [];
    }
    const methodsQuery = select().from('shipping_method');
    methodsQuery.leftJoin('shipping_zone_method').on('shipping_zone_method.method_id', '=', 'shipping_method.shipping_method_id');
    methodsQuery.where('shipping_zone_method.zone_id', '=', zone.shipping_zone_id).and('shipping_zone_method.is_enabled', '=', 't');
    ...
    methods = methods.filter((method)=>{
        ...
        if (method.condition_type === 'price') {
            return toPrice(method.min) <= cart.sub_total && cart.sub_total <= toPrice(method.max);
        } else if (method.condition_type === 'weight') {
            return toPrice(method.min) <= cart.total_weight && cart.total_weight <= toPrice(method.max);
        } else {
            return false;
        }
    });
    // Loop through the methods and calculate the cost
    methods = await Promise.all(methods.map(async (method)=>{
        if (method.calculate_api) {
            ...
            try {
                ...
                throw new Error(`Your shipping calculate API ${method.calculate_api} is invalid`);
            }
            const jsonResponse = await axios.get(api);
            // Detect if the API returns an error base on the http status
            if (jsonResponse.status >= 400) {
                throw new Error(`Error calculating shipping cost for method ${method.name}`);
            }
            return {
                ...method,
            ...
            const weightBasedCost = method.weight_based_cost.map(({ min_weight, cost })=>({
            ...
            const priceBasedCost = method.price_based_cost.map(({ min_price, cost })=>({
