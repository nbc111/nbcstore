...
export default (async (connection)=>{
    await execute(connection, `CREATE TABLE "cart" (
  ...
  CONSTRAINT "CART_UUID_UNIQUE" UNIQUE ("uuid")
    ...
    await execute(connection, `CREATE TABLE "cart_address" (
...
`);
    await execute(connection, `CREATE TABLE "cart_item" (
  ...
  CONSTRAINT "FK_CART_ITEM_PRODUCT" FOREIGN KEY ("product_id") REFERENCES "product" ("product_id") ON DELETE CASCADE ON UPDATE NO ACTION
    ...
    await execute(connection, `CREATE INDEX "FK_CART_ITEM" ON "cart_item" ("cart_id")`);
    await execute(connection, `CREATE INDEX "FK_CART_ITEM_PRODUCT" ON "cart_item" ("product_id")`);
    await execute(connection, `CREATE TABLE "order" (
...
`);
    await execute(connection, `CREATE TABLE "order_activity" (
  ...
  CONSTRAINT "FK_ORDER_ACTIVITY" FOREIGN KEY ("order_activity_order_id") REFERENCES "order" ("order_id") ON DELETE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_ORDER_ACTIVITY" ON "order_activity" ("order_activity_order_id")`);
    await execute(connection, `CREATE TABLE "order_address" (
  ...
  CONSTRAINT "ORDER_ADDRESS_UUID_UNIQUE" UNIQUE ("uuid")
    ...
    await execute(connection, `CREATE TABLE "order_item" (
  ...
  CONSTRAINT "FK_ORDER" FOREIGN KEY ("order_item_order_id") REFERENCES "order" ("order_id") ON DELETE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_ORDER" ON "order_item" ("order_item_order_id")`);
    await execute(connection, `CREATE TABLE "payment_transaction" (
  ...
  CONSTRAINT "FK_PAYMENT_TRANSACTION_ORDER" FOREIGN KEY ("payment_transaction_order_id") REFERENCES "order" ("order_id") ON DELETE CASCADE ON UPDATE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_PAYMENT_TRANSACTION_ORDER" ON "payment_transaction" ("payment_transaction_order_id")`);
    await execute(connection, `CREATE TABLE "shipment" (
  ...
  CONSTRAINT "FK_ORDER_SHIPMENT" FOREIGN KEY ("shipment_order_id") REFERENCES "order" ("order_id") ON DELETE CASCADE
    ...
    await execute(connection, `CREATE INDEX "FK_ORDER_SHIPMENT" ON "shipment" ("shipment_order_id")`);
    // Reduce product stock when order is placed if product manage stock is true
    await execute(connection, `CREATE OR REPLACE FUNCTION reduce_product_stock_when_order_placed()
        RETURNS TRIGGER 
        LANGUAGE PLPGSQL
        AS
      $$
      ...
      END
      $$;`);
    await execute(connection, `CREATE TRIGGER "TRIGGER_AFTER_INSERT_ORDER_ITEM" AFTER INSERT ON "order_item" FOR EACH ROW
