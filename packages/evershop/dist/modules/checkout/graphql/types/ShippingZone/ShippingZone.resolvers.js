...
export default {
    Query: {
        shippingZones: async ()=>{
            const shippingZones = await select().from('shipping_zone').orderBy('shipping_zone_id', 'DESC').execute(pool);
            // Parse the provinces field into an array
            return shippingZones.map((row)=>camelCase(row));
        },
        shippingZone: async (_, { id })=>{
            const shippingZone = await select().from('shipping_zone').where('uuid', '=', id).load(pool);
    ...
    ShippingZone: {
        methods: async (parent)=>{
            ...
            query.innerJoin('shipping_method').on('shipping_method.shipping_method_id', '=', 'shipping_zone_method.method_id');
        ...
        provinces: async ({ shippingZoneId })=>{
            const provinces = await select('province').from('shipping_zone_province').where('zone_id', '=', shippingZoneId).execute(pool);
            return provinces.map((row)=>row.province);
        },
        updateApi: ({ uuid })=>buildUrl('updateShippingZone', {
                id: uuid
            }),
        deleteApi: ({ uuid })=>buildUrl('deleteShippingZone', {
                id: uuid
            }),
        addMethodApi: ({ uuid })=>buildUrl('addShippingZoneMethod', {
                id: uuid
            }),
        removeMethodApi: ({ uuid })=>buildUrl('removeShippingZoneMethod', {
                id: uuid
            })
    },
    ShippingMethodByZone: {
        updateApi: async ({ uuid, zoneId })=>{
            const zone = await select().from('shipping_zone').where('shipping_zone_id', '=', zoneId).load(pool);
