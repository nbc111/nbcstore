...
export default (async (request, response, next)=>{
    try {
        ...
        const products = await select().from('product').where('sku', 'IN', groupedItems.map((item)=>item.sku)).and('status', '=', 1).execute(pool);
        ...
        const cartItems = await Promise.all(groupedItems.map(async (item)=>{
