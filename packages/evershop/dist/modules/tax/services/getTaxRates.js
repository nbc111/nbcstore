...
export async function getTaxRates(taxClassId, country, province, postcode = null) {
    if (!country) {
        return [];
    }
    ...
    if (!taxRates) {
        ...
        taxRates.forEach((taxRate)=>{
            ...
            taxRate.country = taxRate.country ? taxRate.country.split(',').filter((item)=>item.trim() !== '') : [];
            taxRate.province = taxRate.province ? taxRate.province.split(',').filter((item)=>item.trim() !== '') : [];
            taxRate.postcode = taxRate.postcode ? taxRate.postcode.split(',').filter((item)=>item.trim() !== '') : [];
        ...
        taxRates = taxRates.filter((taxRate)=>{
            if ((taxRate.country.includes(country) || taxRate.country.includes('*')) && (taxRate.province.includes(province) || taxRate.province.includes('*') || province === null) && (taxRate.postcode.includes(postcode) || taxRate.postcode.includes('*') || postcode === null)) {
                return true;
            }
            return false;
        });
