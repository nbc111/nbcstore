...
export default (async (connection)=>{
    await execute(connection, `CREATE TABLE "tax_class" (
  ...
  CONSTRAINT "TAX_CLASS_UUID_UNIQUE" UNIQUE ("uuid")
    ...
    const taxClass = await insert('tax_class').given({
        name: 'Taxable Goods'
    }).execute(connection);
    // Add a constraint to product table
    await execute(connection, `ALTER TABLE "product" ADD CONSTRAINT "FK_TAX_CLASS" FOREIGN KEY ("tax_class") REFERENCES "tax_class" ("tax_class_id") ON DELETE SET NULL`);
    // Prevent deleting the default tax class
    await execute(connection, `CREATE OR REPLACE FUNCTION prevent_delete_default_tax_class()
        RETURNS TRIGGER
        LANGUAGE PLPGSQL
        AS
      $$
      ...
      END;
      $$`);
    await execute(connection, `CREATE TRIGGER "PREVENT_DELETING_THE_DEFAULT_TAX_CLASS"
        BEFORE DELETE ON tax_class
        FOR EACH ROW
        EXECUTE PROCEDURE prevent_delete_default_tax_class();`);
    await execute(connection, `CREATE TABLE "tax_rate" (
