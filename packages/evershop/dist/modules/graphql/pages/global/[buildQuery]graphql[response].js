import { execute, NoUnusedFragmentsRule, parse, specifiedRules, validateSchema } from 'graphql';
import { debug } from '../../../../lib/log/logger.js';
import { isDevelopmentMode } from '../../../../lib/util/isDevelopmentMode.js';
import adminSchema, { rebuildSchema } from '../../services/buildSchema.js';
import storeFrontSchema, { rebuildStoreFrontSchema } from '../../services/buildStoreFrontSchema.js';
...
export default async function graphql(request, response, next) {
    ...
    if (isDevelopmentMode()) {
        schema = currentRoute && currentRoute.isAdmin ? await rebuildSchema() : await rebuildStoreFrontSchema();
    } else {
        schema = currentRoute && currentRoute.isAdmin ? adminSchema : storeFrontSchema;
    }
    ...
    try {
        ...
        if (!graphqlQuery) {
            ...
            if (query === 'queryQuery{}') {
                ...
                // Validate the query
                const validationErrors = validateSchema(schema, document, specifiedRules.filter((rule)=>rule !== NoUnusedFragmentsRule));
                if (validationErrors.length > 0) {
                    const formatedErrorMessage = graphqlErrorMessageFormat(graphqlQuery, validationErrors[0].locations[0].line, validationErrors[0].locations[0].column);
