...
export default (async (request, response, next)=>{
    try {
        ...
        if (isDevelopmentMode()) {
            ...
            const queryPath = path.resolve(CONSTANTS.BUILDPATH, subPath, 'server', 'query.graphql');
        ...
        if (query) {
            ...
            if (currentRoute?.isAdmin === false || [
                'widgetNew',
                'widgetEdit'
            ].includes(currentRoute?.id)) {
                applicableWidgets = widgetInstances.map((widget)=>{
                    const widgetSpecs = enabledWidgets.find((enabledWidget)=>enabledWidget.type === widget.type);
                    const componentPath = currentRoute?.isAdmin ? widgetSpecs.settingComponent : widgetSpecs.component;
                    const componentKey = currentRoute?.isAdmin ? widgetSpecs.settingComponentKey : widgetSpecs.componentKey;
                    return {
                        uuid: widget.uuid,
                        type: widget.type,
            ...
            if (applicableWidgets.length > 0) {
                applicableWidgets.forEach((widget)=>{
                    ...
                    let widgetVariables = JSON.parse(JSON.stringify(json.variables[widgetKey]));
                    ...
                    widgetQuery = widgetQuery.replace(regex, (match, p1)=>{
                        ...
                        const path1 = decoded.split(',')[0];
                        const defaultValue = decoded.split(',')[1] || undefined;
                        let value = get(widget.settings, path1, defaultValue);
                        // JSON sringify without adding double quotes to the property name
                        value = JSON5.stringify(value, {
                            quote: '"'
                        });
                    ...
                    if (variableMatch) {
                        widgetQuery = widgetQuery.replace(variableRegex, (match, p1)=>{
                            ...
                            const test = variableList.find((variable)=>variable.origin === `variable_${p1}`);
                            if (test) {
                                return `$${test.new}`;
                            } else {
                    ...
                    const widgetVariablesValues = Object.keys(widgetVariables.values).reduce((acc, key)=>{
                        const check = variableList.find((variable)=>variable.origin === key);
                        if (check) {
                            ...
                            if (typeof v === 'string') {
                                ...
                                if (variableMatch) {
                                    ...
                                    const base64 = variableMatch[0].replace(variableRegex, (match, p1)=>p1);
                                    const decoded = Buffer.from(base64, 'base64').toString('ascii').split(',')[0].replace(/['"]+/g, '');
                                    let actualValue;
                                    if (!decoded.trim()) {
                    ...
                    const widgetVariablesDefs = widgetVariables.defs.reduce((acc, variable)=>{
                        const check = variableList.find((v)=>v.origin === variable.alias);
                        if (check) {
                            acc.push({
                                ...variable,
                                alias: check.new
                    ...
                    queryStr = Object.keys(json.queries).reduce((acc, key)=>{
                        if (!enabledWidgets.find((widget)=>widget.componentKey === key || widget.settingComponentKey === key)) {
                            acc += `\n${json.queries[key]} `;
                        }
                        return acc;
                    }, '');
                    // Now we merge the variables
                    variables = Object.keys(json.variables).reduce((acc, key)=>{
                        if (!enabledWidgets.find((widget)=>widget.componentKey === key || widget.settingComponentKey === key)) {
                            acc.values = {
                                ...acc.values,
                                ...json.variables[key].values
                            };
                            acc.defs = [
                                ...acc.defs,
                                ...json.variables[key].defs
                            ];
                        }
                        return acc;
                    }, {
            ...
            if (variables.defs.length > 0) {
                const variablesString = variables.defs.map((variable)=>`$${variable.alias}: ${variable.type}`).join(', ');
