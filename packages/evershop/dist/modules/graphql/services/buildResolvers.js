...
export async function buildResolvers(isAdmin = false) {
    ...
    extensions.forEach((extension)=>{
        typeSources.push(path.join(extension.path, 'graphql/types/**/*.resolvers.{js,ts}'));
    });
    // Using loadFiles with an array of glob patterns instead of joining them
    const resolvers = mergeResolvers(await loadFiles(typeSources, {
        ...
        requireMethod: async (path)=>{
            if (isDevelopmentMode()) {
                const module = await import(`${url.pathToFileURL(path)}?t=${Date.now()}`);
                return module;
            } else {
