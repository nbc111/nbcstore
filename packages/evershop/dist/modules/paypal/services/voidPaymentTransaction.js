...
export async function voidPaymentTransaction(orderID) {
    try {
        const transaction = await select().from('payment_transaction').where('payment_transaction_order_id', '=', orderID).load(pool);
        if (!transaction) {
            return;
        }
        ...
        const responseData = await axiosInstance.get(`/v2/payments/authorizations/${transaction.transaction_id}`);
        ...
        if (responseData.data.status === 'VOIDED') {
            ...
            await axiosInstance.post(`/v2/payments/authorizations/${transaction.transaction_id}/void`);
        } else {
