import { getConnection, insert, select } from '@evershop/postgres-query-builder';
...
export default (async (request, response, next)=>{
    try {
        ...
        if (!order) {
            ...
            const responseData = await axiosInstance.post(`/v2/checkout/orders/${order.integration_order_id}/authorize`);
            if (responseData.data.status === 'COMPLETED') {
                ...
                await addOrderActivityLog(order.order_id, `Customer authorized the payment using PayPal. Transaction ID: ${responseData.data.purchase_units[0].payments.authorizations[0].id}`, false, await getConnection(pool));
                response.status(OK);
                response.json({
                    data: {}
                });
