...
export default (async (request, response, next)=>{
    try {
        ...
        const order = await select().from('order').where('uuid', '=', order_id).and('payment_method', '=', 'paypal').and('payment_status', '=', 'pending').load(pool);
        if (!order) {
            ...
            const amount = {
                ...
                breakdown: {
                    item_total: {
                        currency_code: order.currency,
                        value: catalogPriceInclTax ? toPrice(order.sub_total_incl_tax) : toPrice(order.sub_total)
                    },
                    shipping: {
                        currency_code: order.currency,
                        value: catalogPriceInclTax ? toPrice(order.shipping_fee_incl_tax) : toPrice(order.shipping_fee_excl_tax)
                    },
                    discount: {
                        currency_code: order.currency,
                ...
                purchase_units: [
                    {
                        items: items.map((item)=>({
                                ...
                                unit_amount: {
                                    currency_code: order.currency,
                                    value: catalogPriceInclTax ? toPrice(item.final_price_incl_tax) : toPrice(item.final_price)
                                },
                                category: item.no_shipping_required ? 'DIGITAL_GOODS' : 'PHYSICAL_GOODS'
                            })),
                        amount: finalAmount
                    }
                ],
                application_context: {
                    cancel_url: `${getContextValue(request, 'homeUrl', '')}${buildUrl('paypalCancel', {
                        order_id
                    })}`,
                    return_url: `${getContextValue(request, 'homeUrl', '')}${buildUrl('paypalReturn', {
                        order_id
                    })}`,
            ...
            const finalPaypalOrderData = getValueSync('finalPaypalOrderData', orderData, {
                order,
                items,
                shippingAddress
            });
            ...
            const { data } = await axiosInstance.post(`/v2/checkout/orders`, finalPaypalOrderData, {
                validateStatus: (status)=>status < 500
            });
            if (data.id) {
                // Update order and insert papal order id
                await update('order').given({
                    integration_order_id: data.id
                }).where('uuid', '=', order_id).execute(pool);
                response.status(OK);
                ...
                await update('cart').given({
                    status: true
                }).where('cart_id', '=', order.cart_id).execute(pool);
