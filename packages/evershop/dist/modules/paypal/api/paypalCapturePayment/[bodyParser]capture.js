...
export default (async (request, response, next)=>{
    ...
    try {
        ...
        if (!order) {
            ...
            const responseData = await axiosInstance.post(`/v2/checkout/orders/${order.integration_order_id}/capture`);
            if (responseData.data.status === 'COMPLETED') {
                ...
                await addOrderActivityLog(order.order_id, `Captured the payment. Transaction ID: ${responseData.data.purchase_units[0].payments.captures[0].id}`, false, connection);
