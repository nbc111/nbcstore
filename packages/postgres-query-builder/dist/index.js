...
class Node {
    ...
    addLeaf(link, field, operator, value) {
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            ...
            this._tree.push(new Leaf(link, field, operator, {
                value: value,
                isSQL: this._defaultValueTreatment === 'sql'
            }, this));
        }
        return this;
    }
    ...
    getLeafs() {
        return this._tree.filter((e)=>e instanceof Leaf || e instanceof RawLeaf);
    }
    getNodes() {
    ...
    findLeaf(link, field, operator, value) {
        for (const element of this._tree){
            if (element instanceof Leaf && element._link === link && element._field === fieldResolve(field) && element._binding[field] === value) {
                return element;
    ...
    getBinding() {
        ...
        this._tree.forEach((element)=>{
            if (element instanceof Leaf || element instanceof RawLeaf || element instanceof Node) {
    ...
    async execute(connection, releaseConnection = true) {
        ...
        try {
            ...
            if (e.code === '42703') {
                ...
                const countField = this._select._fields.find((f)=>/COUNT\s*\(/i.test(f));
                if (countField) {
                    const alias = countField.match(/(?<=as\s)(.*)/i);
                    let aliasResult = '';
                    if (alias) {
      ...
      WHERE table_name = '${this._table}'`);
        ...
        rows.forEach((field)=>{
            if ([
                'BY DEFAULT',
                'ALWAYS'
            ].includes(field['identity_generation'])) {
...
export { select, insert, update, node, del, insertOnUpdate, getConnection, startTransaction, commit, rollback, release, execute, sql, value, // Types for external usage
SelectQuery, UpdateQuery, InsertQuery, InsertOnUpdateQuery, DeleteQuery, Pool };
